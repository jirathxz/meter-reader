...
เวอร์ชัน 1.1: Functional Pipeline ล้วน (ไม่มี OOP) — กระชับ อ่านจากบนลงล่างได้ทันที

ลำดับการทำงาน 4 ขั้นตอน:
  1. check_water_meter: SigLIP2 Zero-shot ตรวจสอบว่าเป็นภาพมิเตอร์น้ำจริง
  2. detect_digits_best: หมุน 4 ทิศ × 3 ฟิลเตอร์ ค้นหามุมและภาพที่ดีที่สุด
  3. is_vertical & dedup_detections: กรองกล่องซ้อนและแยกแยะแถวแนวนอนออกจากคอลัมน์แนวตั้ง
  4. flip_guard & cross_check_digits: ตรวจสอบความถูกต้อง (Flip-guard 180°, SigLIP2 Cross-check, Red Dial Ratio)
...
# ================================================================
# 1) ค่าคงที่ (Constants) — ปรับตรงนี้ที่เดียว
# ================================================================
...
# ================================================================
# 2) โหลดโมเดลแบบ Lazy (โหลดครั้งแรกเมื่อถูกเรียกใช้)
# ================================================================
...
def get_siglip():
    ...
    if _siglip is None:
        ...
        _siglip = (
            AutoProcessor.from_pretrained(SIGLIP_MODEL),
            AutoModel.from_pretrained(SIGLIP_MODEL).to(DEVICE).eval(),
        )
    return _siglip
...
def rotate_image(img_bgr: np.ndarray, angle: int) -> np.ndarray:
    """หมุนภาพ 90, 180, 270 องศา (0 = ภาพเดิม)"""
...
def apply_prep(img_bgr: np.ndarray, prep: str) -> np.ndarray:
    """ปรับคอนทราสต์: CLAHE บน LAB หรือ HistEq บน YCrCb"""
    if prep == "clahe":
...
def iou(b1: list[float], b2: list[float]) -> float:
    """คำนวณ IoU ของกล่อง Bounding Box 2 กล่อง"""
...
def dedup_detections(dets: list[dict[str, Any]], thresh: float = 0.45) -> list[dict[str, Any]]:
    """กรองกล่องซ้อนทับกัน (IoU > thresh) โดยเก็บตัวที่ confidence สูงสุด"""
    kept = []
...
def remap_bbox(bbox: list[float], angle: int, w: int, h: int) -> list[float]:
    """แปลงพิกัดจากภาพหมุนกลับสู่พิกัดภาพตั้งต้น"""
    x1, y1, x2, y2 = bbox
    remap_dict = {
        0: bbox,
        90: [y1, h - x2, y2, h - x1],
...
def red_ratio(img_bgr: np.ndarray, bbox: list[float]) -> float:
    """คำนวณสัดส่วนสีแดงในกล่องตัวเลข (หลักแดงอยู่ขวาสุดของมิเตอร์เสมอ)"""
...
# ================================================================
# 4) ตรวจจับตัวเลขและประเมินทิศทาง (Detection & Alignment)
# ================================================================

def detect_digits(img_bgr: np.ndarray) -> list[dict[str, Any]]:
    """YOLO หาตัวเลข 0-9 เรียงลำดับจากซ้ายไปขวา"""
...
def eval_orientation(bgr_img: np.ndarray, angle: int, prep: str):
...
# ================================================================
# 5) ตรวจสอบความถูกต้อง & กันกลับหัว (Verification Guards)
# ================================================================
...
def flip_guard(rgb_img: np.ndarray, digits: list[dict[str, Any]], meta: dict[str, Any] | None, h: int, w: int) -> dict[str, Any]:
    """อ่านซ้ำแบบ 180° ตรวจสอบความสมมาตรป้องกันอ่านกลับหัว"""
    ...
    rot = rotate_image(cv2.cvtColor(rgb_img, cv2.COLOR_RGB2BGR), anti_a)
    anti_dets = dedup_detections(detect_digits(apply_prep(rot, meta["prep"])))
    if not anti_dets:
...
def cross_check_digits(rgb_img: np.ndarray, digits: list[dict[str, Any]], h: int, w: int, angle: int = 0) -> dict[str, Any]:
    """SigLIP2 ตรวจทานเฉพาะหลักที่ YOLO มีค่าความมั่นใจต่ำ (< 0.60)"""
...
# ================================================================
# 6) ไปป์ไลน์หลัก (Main Pipeline Entry Point)
# ================================================================
...
def read_meter(rgb_img: np.ndarray) -> dict[str, Any]:
    """ฟังก์ชันหลัก: รับภาพ RGB เข้ามา → ทำการคัดแยก → หาตัวเลข → คืนค่าผลลัพธ์"""
    t0 = perf_counter()
    h, w = rgb_img.shape[:2]

    # ขั้นที่ 1: ตรวจว่าใช่มิเตอร์น้ำจริงไหม
    ...
    # ขั้นที่ 2: หามุมที่ดีที่สุดและตรวจจับตัวเลข
    ...
    # ขั้นที่ 3: กรองกรณีกล่องเรียงตัวเป็นคอลัมน์แนวตั้ง
    ...
    # ขั้นที่ 4: ตรวจสอบความถูกต้องและสร้าง Warnings
