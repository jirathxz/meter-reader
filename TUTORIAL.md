...
- [บทที่ 1: ทฤษฎีพื้นฐาน เทคโนโลยีที่ใช้ และการประมวลผลภาพ](#บทที่-1-ทฤษฎีพื้นฐาน-เทคโนโลยีที่ใช้-และการประมวลผลภาพ)
  ...
  - [1.3 วัตถุประสงค์ของระบบ](#13-วัตถุประสงค์ของระบบ)
  - [1.4 ขอบเขตการศึกษา](#14-ขอบเขตการศึกษา)
  - [1.5 ผลที่คาดว่าจะได้รับ](#15-ผลที่คาดว่าจะได้รับ)
  - [1.6 ระยะเวลาดำเนินงาน](#16-ระยะเวลาดำเนินงาน)
  - [1.7 ทฤษฎีและหลักการพื้นฐาน](#17-ทฤษฎีและหลักการพื้นฐาน)
    - [1.7.1 คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ](#171-คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ)
    - [1.7.2 การจำแนกประเภทภาพแบบซีโร่ช็อต (Zero-shot Classification)](#172-การจำแนกประเภทภาพแบบซีโร่ช็อต-zero-shot-classification)
    - [1.7.3 การประมวลผลภาพดิจิทัล](#173-การประมวลผลภาพดิจิทัล)
  - [1.8 เทคโนโลยีและไลบรารีที่ใช้ในโครงงาน](#18-เทคโนโลยีและไลบรารีที่ใช้ในโครงงาน)
  - [1.9 กระบวนการทำงานของผู้ใช้และภาพรวมระบบ](#19-กระบวนการทำงานของผู้ใช้และภาพรวมระบบ)
  ...
  - [1.12 การคัดกรองภาพด้วยแบบจำลอง SigLIP2](#112-การคัดกรองภาพด้วยแบบจำลอง-siglip2)
  - [1.13 การตรวจจับและอ่านค่าตัวเลขด้วยคอมพิวเตอร์วิทัศน์](#113-การตรวจจับและอ่านค่าตัวเลขด้วยคอมพิวเตอร์วิทัศน์)
...
&emsp;&emsp;&emsp;&emsp;ตารางที่ 1.0 แสดงแผนการดำเนินงานของโครงงานตลอดระยะเวลา 16 สัปดาห์

**ตารางที่ 1.0 แผนการดำเนินงาน (Gantt Chart)**
...
### 1.8 เทคโนโลยีและไลบรารีที่ใช้ในโครงงาน

&emsp;&emsp;&emsp;&emsp;ระบบอ่านค่ามาตรวัดน้ำอัตโนมัตินี้ประยุกต์ใช้ไลบรารีและเทคโนโลยีรวมทั้งสิ้น 10 รายการ โดยแต่ละรายการได้รับการคัดเลือกตามความเหมาะสมกับบทบาทเฉพาะในระบบ ดังตารางที่ 1.1

&emsp;&emsp;&emsp;&emsp;**ตารางที่ 1.1 เทคโนโลยีและไลบรารีที่ใช้ในโครงงาน**

| ไลบรารี / เทคโนโลยี | บทบาทในระบบนี้ | เหตุผลที่เลือกใช้ (ไม่ใช้ทางเลือกอื่น) |
|---|---|---|
| **`ultralytics` (YOLO26)** | ตรวจจับและระบุตัวเลข 0–9 จากภาพถ่ายเต็มโดยตรง | เร็วที่สุดในกลุ่ม real-time object detection ไม่ต้องครอปภาพก่อน ต่างจาก Tesseract |
| **`transformers` (SigLIP2)** | คัดกรองว่าภาพที่รับมาเป็นมาตรวัดน้ำหรือไม่ (Zero-shot) | ไม่ต้องฝึกแบบจำลองใหม่ รองรับการจำแนกหลายประเภทด้วยข้อความภาษาธรรมชาติ |
| **`opencv-python`** | หมุนภาพ, ปรับแสง CLAHE และ HistEq, ตรวจสีแดง HSV | มาตรฐานอุตสาหกรรม Computer Vision รองรับการประมวลผลภาพครบทุกด้าน |
| **`torch`** | คำนวณ Neural Network ทั้ง YOLO และ SigLIP2 | รองรับการเร่งความเร็วด้วย GPU/CPU อัตโนมัติ เป็นพื้นฐานของทั้งสองแบบจำลอง |
| **`fastapi`** | ให้บริการ REST API รับ-ส่งภาพผ่าน HTTP | เร็วกว่า Flask/Django มีระบบสร้างเอกสาร Swagger UI อัตโนมัติ รองรับ Async |
| **`gradio`** | สร้างหน้าเว็บทดสอบระบบสำหรับผู้ใช้ทั่วไป | ออกแบบมาเพื่อ AI demo โดยเฉพาะ สร้าง UI ได้ในไม่กี่บรรทัดโค้ด |
| **`pillow`** | อ่านและแปลงไฟล์ภาพ JPG/PNG/WEBP เป็น NumPy Array | มาตรฐาน Python image library รองรับรูปแบบไฟล์ภาพหลากหลาย |
| **`numpy`** | คำนวณ Matrix และ Array ตลอดกระบวนการประมวลผล | พื้นฐานสำหรับ OpenCV, YOLO และ SigLIP2 ทุกขั้นตอน |
| **`uvicorn`** | เว็บเซิร์ฟเวอร์ ASGI สำหรับ FastAPI | รองรับ Async I/O สูง เหมาะกับงานประมวลผลภาพที่ใช้เวลานาน |
| **`uv`** | จัดการ Virtual Environment และติดตั้งไลบรารี | เร็วกว่า `pip` หลายสิบเท่า พัฒนาด้วยภาษา Rust รองรับ Python 3.11 |

---

### 1.9 กระบวนการทำงานของผู้ใช้และภาพรวมระบบ

&emsp;&emsp;&emsp;&emsp;ก่อนศึกษารายละเอียดทางเทคนิค ผู้ศึกษาควรทำความเข้าใจกระบวนการทำงานของระบบในมุมมองของผู้ใช้งานก่อน เพื่อให้เห็นภาพรวมของระบบทั้งหมด

&emsp;&emsp;&emsp;&emsp;**กระบวนการทำงานของผู้ใช้ (User Flow):** ผู้ใช้งาน**ถ่ายภาพ**มาตรวัดน้ำด้วยกล้องสมาร์ทโฟน แล้ว**ส่งภาพ**เข้าสู่ระบบผ่านหน้าเว็บ Gradio หรือ FastAPI จากนั้น**ระบบประมวลผล**ภาพและ**ส่งค่าตัวเลข**ที่อ่านได้พร้อมค่าความเชื่อมั่นกลับมาให้ผู้ใช้

```mermaid
flowchart LR
  A["📷 ผู้ใช้<br/>ถ่ายภาพมาตรวัดน้ำ"] --> B["🌐 อัปโหลดภาพ<br/>ผ่าน Gradio หรือ API"]
  B --> C["⚙️ ระบบประมวลผล<br/>SigLIP2 → YOLO → Safety"]
  C --> D["📊 ผู้ใช้รับค่าตัวเลข<br/>พร้อม confidence และ warnings"]
```
...
#### 1.7.1 คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ
...
&emsp;&emsp;&emsp;&emsp;โครงงานนี้ประยุกต์ใช้แบบจำลอง **YOLO (You Only Look Once)** ซึ่งเป็นแบบจำลองปัญญาประดิษฐ์สำหรับการตรวจจับวัตถุที่มีความเร็วระดับเวลาจริง (Real-time Object Detection) โดยมีหลักการ **มองภาพเพียงครั้งเดียวแล้วตอบทันที** แตกต่างจากแบบจำลองสองขั้นตอน (Two-stage) เช่น Faster R-CNN YOLO แบ่งภาพเป็นกริด ทำนายกรอบและประเภทพร้อมกันในรอบเดียว จึงเร็วและเหมาะกับงานที่ต้องประมวลผลต่อเนื่อง (Jocher et al., 2026; Zou et al., 2023)
...
#### 1.7.2 การจำแนกประเภทภาพแบบซีโร่ช็อต (Zero-shot Classification)
...
### 1.12 การคัดกรองภาพด้วยแบบจำลอง SigLIP2
...
### 1.13 การตรวจจับและอ่านค่าตัวเลขด้วยคอมพิวเตอร์วิทัศน์
