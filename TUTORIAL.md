...
&emsp;&emsp;&emsp;&emsp;1.6.2.3 วัตถุประสงค์ในการประยุกต์ใช้แบบจำลอง YOLO: โครงงานเลือกใช้แบบจำลอง YOLO เพื่อลดการพึ่งพาขั้นตอนการตัดภาพบริเวณหน้าปัดล่วงหน้า ซึ่งแตกต่างจากสถาปัตยกรรมแบบสองขั้นตอน (Two-stage Cascaded Pipeline) ในงานเดิมที่ต้องตัดส่วนหน้าปัดด้วยแบบจำลองเซกเมนต์ก่อนส่งต่อให้ตัวตรวจจับตัวเลข (อาทิ สถาปัตยกรรมที่ผสาน UNet และ Faster R-CNN ของ Radyhin, 2022) การใช้ YOLO ช่วยให้ระบบตรวจจับตำแหน่งและจำแนกตัวเลขจากภาพเต็มเฟรมได้ภายในขั้นตอนเดียว โดยความสามารถในการรองรับภาพที่มีความเอียงหรือสภาพแวดล้อมซับซ้อนขึ้นอยู่กับข้อมูลฝึกและขั้นตอนประมวลผลที่ใช้ในโครงงาน
...
| Faster R-CNN (ResNet-50) | ~41M | ~6.5 GB+ | สถาปัตยกรรม Two-stage แม่นยำกับวัตถุซ้อนทับ | โครงสร้างซับซ้อน อัตราการประมวลผลช้าเกินไปสำหรับงานหน้างาน | Ren et al. (2015); Radyhin (2022) |
...
<p align="center"><em>ที่มา: ปรับปรุงจาก Ultralytics (2026), Ren et al. (2015), Smith (2007), และ Radyhin (2022)</em><br><em>* หมายเหตุ: ค่า VRAM ในตารางเป็นค่าโดยประมาณจากการทดลองของโครงงาน ค่าดังกล่าวขึ้นอยู่กับ batch size, image size และสภาพแวดล้อมของการรัน</em></p>
...
> [!NOTE]
> **ข้อจำกัดของข้อมูลในตารางที่ 1.1:** ตัวเลขจำนวนพารามิเตอร์ในตารางข้างต้นอ้างอิงตามเอกสารเผยแพร่ของผู้พัฒนาและวรรณกรรมเดิม (Ultralytics, 2026; Radyhin, 2022; Ren et al., 2015; Smith, 2007) ขณะที่ตัวเลขขนาดหน่วยความจำ VRAM เป็นค่าสังเกตจากการทดลองฝึกฝนจริงของโครงงานบน GPU NVIDIA Tesla T4 (VRAM 16 GB) ด้วยขนาดแบทช์ batch=16 และความละเอียดภาพ imgsz=960 ซึ่งการใช้ VRAM จริงจะแปรผันตามขนาดแบทช์ ขนาดภาพ และฮาร์ดแวร์ที่ใช้ โดยค่าดังกล่าวขึ้นอยู่กับ batch size, image size และสภาพแวดล้อมของการรัน ทั้งนี้ตารางข้างต้นไม่ได้รวมเวลาประมวลผล (Latency) เนื่องจากสภาวะแวดล้อมในการทดสอบของแต่ละเอกสารมีความแตกต่างกันอย่างมาก โดยผลการวัดเวลาจริงของระบบปลายทางถึงปลายทางบนโปรเจกต์นี้ มีการรายงานอย่างละเอียดและควบคุมสภาวะแวดล้อมในหัวข้อ 4.1.4 โดยจำแนกเวลา Preprocessing, Inference, และ Postprocessing ภายใต้ฮาร์ดแวร์ที่ระบุชัดเจน
