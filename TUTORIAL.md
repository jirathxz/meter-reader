...
&emsp;&emsp;&emsp;&emsp;2. แบบจำลอง YOLO มีข้อดีอย่างไร: สถาปัตยกรรม YOLO ทำงานแบบขั้นตอนเดียว (Single-stage) ซึ่งทำนายพิกัดกรอบและคลาสตัวเลขพร้อมกันในรอบการคำนวณเดียว ตามเอกสารของ Ultralytics (2026) ระบุว่าใน YOLO26 ได้รับการออกแบบให้ใช้ส่วนหัวตรวจจับแบบไร้กล่องสมอ (Anchor-free Detection Head) และรองรับการอนุมานแบบไม่ต้องพึ่งพา Non-Maximum Suppression (NMS-free inference) เพื่อลด Latency ในขั้นตอนประมวลผลภายหลัง ขณะเดียวกันยังคงรองรับเส้นทางการอนุมานที่ใช้ NMS ตามปกติสำหรับการปรับจูนเกณฑ์ความแม่นยำ โดยประสิทธิภาพในการตรวจจับวัตถุขนาดเล็กขึ้นอยู่กับสถาปัตยกรรม การฝึก และข้อมูลที่ใช้ ทั้งนี้ เวลาประมวลผลขึ้นอยู่กับฮาร์ดแวร์ ความละเอียดภาพ และจำนวนสมมติฐานที่ใช้ โดยโครงงานนี้รายงานเวลาประมวลผลจริงอย่างละเอียดในหัวข้อ 4.1.4 (จำแนกตามขั้นตอน Preprocessing, Inference และ Postprocessing) นอกจากนี้ ในขั้นตอนการฝึกฝน โครงงานได้กำหนดเทคนิค Data Augmentation ใน `training/yolo_train.py` (อาทิ Mosaic, Mixup, การหมุนองศา และการปรับค่า HSV) เพื่อเพิ่มความทนทานของแบบจำลองต่อสภาพแวดล้อมที่หลากหลาย
...
&emsp;&emsp;&emsp;&emsp;การจำแนกประเภทภาพแบบไม่ใช้ภาพตัวอย่าง หรือ Zero-shot (Zero-shot Image Classification) เป็นความสามารถของแบบจำลองปัญญาประดิษฐ์ในการจำแนกประเภทของภาพถ่ายตามคำอธิบายภาษาธรรมชาติ (Natural Language Prompts) โดยที่แบบจำลองไม่เคยได้รับการฝึกฝนด้วยชุดข้อมูลภาพของคลาสนั้นมาก่อน โดยแนวคิดการใช้ Sigmoid Loss สำหรับการเรียนรู้ภาพ-ภาษา (SigLIP) ได้รับการบุกเบิกโดย Zhai et al. (2023) และต่อมาได้รับการต่อยอดสถาปัตยกรรมพร้อมกระบวนการฝึกฝนแบบ Self-supervised ในแบบจำลอง SigLIP 2 โดย Tschannen et al. (2025)
...
&emsp;&emsp;&emsp;&emsp;1) *ค่าที่วัดได้จริง:* เวลารวมทั้ง 7 ภาพเท่ากับ 72,339 มิลลิวินาที ส่งผลให้เวลาเฉลี่ยเท่ากับ 10,334 ms (10.33 วินาทีต่อภาพ) ตามข้อมูลดิบในตารางที่ 4.4<br>
...
&emsp;&emsp;&emsp;&emsp;1) *ค่าที่วัดได้จริง:* ค่าเฉลี่ย 5,828.8 ms (ประมาณ 5.8 วินาทีต่อภาพ) ในขั้นตอน M7 ของตารางที่ 4.5 และสอดคล้องตรงกันกับค่าเฉลี่ยเฉพาะภาพที่ 2 ถึง 7 ของตารางที่ 4.4 (34,942 ms / 6 ภาพ = 5,823.7 ms หรือ 5.82 วินาทีต่อภาพ)<br>
...
&emsp;&emsp;&emsp;&emsp;ดังนั้น โครงงานนี้จึงกำหนดข้อสรุปอย่างระมัดระวัง โดยระบุว่าความแม่นยำ 100.0% เป็นผลการทำงานเฉพาะบนชุดตัวอย่างสาธิต $N=7$ เท่านั้น และนำผลการประเมินประสิทธิภาพตัวตรวจจับวัตถุบนชุดทดสอบที่กันออกจากการฝึก $N=194$ (ตารางที่ 4.6) มาเป็นหลักฐานสนับสนุนความสามารถในการตรวจจับร่วมด้วย
...
| การค้นหา 12 สมมติฐานสัมพันธ์กับอัตราความแม่นยำที่สูงกว่า Single-pass บนชุดการทดลอง | Empirical | ผลการทดลอง Progressive Ablation: M0 (42.9%) vs M2 (71.4%) vs M4 (100.0%) (ดูตารางที่ 4.5 ในหัวข้อ 4.1.3) | Supported |
...
| YOLO26m มีประสิทธิภาพการตรวจจับสูงบนชุดทดสอบ | Empirical | Precision, Recall, mAP@50, mAP@50-95 บนชุดทดสอบอิสระ 194 ภาพ (ดูตารางที่ 4.6 ในหัวข้อ 4.1.5) | Supported |
| เวลาประมวลผลของระบบบน CPU (Steady-state Latency 5.83 s/ภาพ) | Empirical | CPU Benchmark บน AMD Ryzen 5 8645HS: M0=421.1ms, M1=1,710.3ms, M7=5,828.8ms (ดูหัวข้อ 4.1.4 และตารางที่ 4.5) | Supported |
| สถาปัตยกรรมแบบไฮบริด (YOLO + SigLIP2) มีเหตุผลเชิงสถาปัตยกรรมที่เหมาะสมสำหรับการทดลองในบริบทภาคสนาม | Architectural | ผสานความเร็วและความแม่นยำของ YOLO เข้ากับความยืดหยุ่นทางภาษาภาพแบบ Zero-shot ของ SigLIP2 เพื่อสนับสนุนการทดลองในสภาพแวดล้อมภาคสนาม | Design Rationale |
