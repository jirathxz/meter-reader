...
**รายละเอียด implementation (เก็บไว้อ่านรอบสอง):** สูตร `y, h-x` / `w-x, h-y` / `w-y, x` คือการสลับแกนตามองศา โดย `w, h` คือขนาดภาพต้นฉบับ (มุมซ้ายบนเป็น 0,0) และ `remap_bbox` แค่เรียก `remap_point` สองครั้งแล้ว `min/max` — ทั้งสองสูตรตรงกับโค้ดจริงใน `main.py:remap_point()` (บรรทัด 121–129) และ `remap_bbox()` (132–142) แบบ 1:1
