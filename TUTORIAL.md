...
&emsp;&emsp;&emsp;&emsp;[3.1.1 การตรวจสอบความคงเส้นคงวาจากการหมุน 180 องศาด้วยฟังก์ชัน `flip_guard`](#311-การตรวจสอบความคงเส้นคงวาจากการหมุน-180-องศาด้วยฟังก์ชัน-flip_guard-180-rotation-consistency-check)  
...
&emsp;&emsp;[3.4 การทดสอบระบบและการวิเคราะห์ปัญหา](#34-การทดสอบระบบและการวิเคราะห์ปัญหา)  
...
&emsp;&emsp;&emsp;&emsp;[3.4.3 สรุปผลการทดสอบการทำงานของระบบเบื้องต้น](#343-สรุปผลการทดสอบการทำงานของระบบเบื้องต้น)  
...
&emsp;&emsp;&emsp;&emsp;โครงงานนี้เลือกใช้แบบจำลอง YOLO (You Only Look Once) ซึ่งเป็นสถาปัตยกรรมโครงข่ายประสาทเทียมสำหรับการตรวจจับวัตถุแบบขั้นตอนเดียว (Single-stage Object Detector) ที่ได้รับการพัฒนาและใช้งานอย่างแพร่หลายในงานคอมพิวเตอร์วิทัศน์เชิงประยุกต์ โดยแบบจำลองจะรับข้อมูลภาพเข้าสู่โครงข่ายและทำนายพิกัดกรอบขอบเขตพร้อมประเภทของวัตถุทั้งหมดได้ในรอบการคำนวณเดียวตามสถาปัตยกรรมของ Ultralytics (Jocher et al., 2023; Ultralytics, 2026; Zou et al., 2023)
...
&emsp;&emsp;&emsp;&emsp;2. แบบจำลอง YOLO มีข้อดีอย่างไร: สถาปัตยกรรม YOLO ทำงานแบบขั้นตอนเดียว (Single-stage) ซึ่งทำนายพิกัดกรอบและคลาสตัวเลขพร้อมกันในรอบการคำนวณเดียว ตามเอกสารของ Ultralytics (2026) สถาปัตยกรรม YOLO รุ่นใหม่ใช้การออกแบบแบบไร้กล่องสมอ (Anchor-free) โดยการออกแบบแบบ Anchor-free ช่วยลดการพึ่งพากล่องต้นแบบ โดยประสิทธิภาพในการตรวจจับวัตถุขนาดเล็กขึ้นอยู่กับสถาปัตยกรรม การฝึก และข้อมูลที่ใช้ ทั้งนี้ เวลาประมวลผลขึ้นอยู่กับฮาร์ดแวร์ ความละเอียดภาพ และจำนวนสมมติฐานที่ใช้ โดยโครงงานนี้รายงานเวลาประมวลผลจริงอย่างละเอียดในหัวข้อ 4.1.4 (จำแนกตามขั้นตอน Preprocessing, Inference และ Postprocessing) นอกจากนี้ ในขั้นตอนการฝึกฝน โครงงานได้กำหนดเทคนิค Data Augmentation ใน `training/yolo_train.py` (อาทิ Mosaic, Mixup, การหมุนองศา และการปรับค่า HSV) เพื่อเพิ่มความทนทานของแบบจำลองต่อสภาพแวดล้อมที่หลากหลาย
...
| YOLO26 Small | ~11M | ~3.0 GB | ความสามารถในการตรวจจับรายละเอียดสูงขึ้น | ขนาดแบบจำลองและสมรรถนะการสกัดคุณลักษณะระดับกลาง | Ultralytics (2026) |
...
#### 3.1.1 การตรวจสอบความคงเส้นคงวาจากการหมุน 180 องศาด้วยฟังก์ชัน `flip_guard` (180° Rotation Consistency Check)
...
def flip_guard(rgb_img: np.ndarray, digits: list[dict[str, Any]], meta: dict[str, Any] | None) -> dict[str, Any]:
    """ตรวจสอบความคงเส้นคงวาจากการหมุน 180° ป้องกันการอ่านกลับหัว (180° Rotation Consistency Check)"""
...
### 3.4 การทดสอบระบบและการวิเคราะห์ปัญหา
...
```
<p align="center"><strong>ชุดคำสั่งที่ 3.1: Terminal 1 (1/3) — การสั่งเริ่มต้นทำงานเซิร์ฟเวอร์ FastAPI Backend ด้วยเครื่องมือ <code>uv</code></strong></p>
...
```
<p align="center"><strong>ชุดคำสั่งที่ 3.2: Terminal 2 (2/3) — การสั่งเริ่มต้นทำงานส่วนติดต่อผู้ใช้ Gradio Web UI ด้วยเครื่องมือ <code>uv</code></strong></p>
...
3.4.1.3 การทดสอบระบบด้วยชุดคำสั่งอัตโนมัติ (Automated Evaluation Scripts): ผู้พัฒนาสามารถสั่งรันชุดคำสั่งทดสอบระบบอ่านค่าแบบครบวงจร เพื่อยืนยันความถูกต้องของอัลกอริทึมได้ทันทีผ่าน Terminal ดังแสดงในชุดคำสั่งที่ 3.3:

```powershell
# รันการทดสอบและประเมินความแม่นยำของระบบอ่านค่าอัตโนมัติบนชุดภาพตัวอย่าง
uv run python validate.py

# รันการทดสอบเชิงตัดทอน (Ablation Study) เปรียบเทียบ 5 โหมดประมวลผล
uv run python validate_ablation.py
```
<p align="center"><strong>ชุดคำสั่งที่ 3.3: Terminal (3/3) — การสั่งประเมินผลระบบและทดสอบเชิงตัดทอนด้วยสคริปต์อัตโนมัติ</strong></p>

&emsp;&emsp;&emsp;&emsp;ผลลัพธ์ที่คาดหวัง: สคริปต์จะทำการโหลดภาพทดสอบในโฟลเดอร์ `meter_img/` ส่งผ่านกระบวนการประมวลผลทั้งหมด และบันทึกผลการประเมินลงในไฟล์ `reviews/validation_results.json` และ `reviews/ablation.json` โดยอัตโนมัติ
...
#### 3.4.3 สรุปผลการทดสอบการทำงานของระบบเบื้องต้น

&emsp;&emsp;&emsp;&emsp;เพื่อยืนยันการทำงานของระบบหลังการติดตั้งและรันสคริปต์ทดสอบ `validate.py` ตารางที่ 3.2 สรุปผลสัมฤทธิ์เบื้องต้นของระบบอ่านค่ามาตรวัดน้ำอัตโนมัติจากการประเมินบนชุดภาพสาธิตภาคสนามและชุดทดสอบที่กันออกจากการฝึก โดยจัดลำดับรายงานตามระดับความสำคัญของตัวชี้วัดความถูกต้องที่แท้จริง ก่อนตัวชี้วัดความเร็วและค่าความเชื่อมั่น:

<p align="center"><strong>ตารางที่ 3.2: สรุปผลการทดสอบการทำงานของระบบเบื้องต้นสำหรับผู้พัฒนา</strong></p>

| ลำดับ | ตัวชี้วัดประสิทธิภาพ (Metric) | ชุดสาธิตภาคสนาม (Demo Set, n=7) | ชุดทดสอบที่กันออกจากการฝึก (Test Split, n=120) | บทบาทและความหมายเชิงวิศวกรรม |
|:---:|---|:---:|:---:|---|
| 1 | **Exact Reading Accuracy** | **100.0% (7/7)** | **67.5% (81/120)** | ตัวชี้วัดความถูกต้องหลัก: อ่านถูกครบทุกหลัก 100% |
| 2 | **Digit-level Accuracy** | **100.0% (36/36)** | **90.44% (851/941)** | สัดส่วนความถูกต้องรายหลักตัวเลข |
| 3 | **Digit Error Rate (DER)** | **0.00% (0.0000)** | **7.33% (0.0733)** | อัตราความผิดพลาดของหลักตัวเลข $(S+D+I)/N_{	ext{ref}}$ |
| 4 | **Detection mAP@50 (YOLO26m)** | — | **87.86%** | ประสิทธิภาพของตัวตรวจจับวัตถุบน Test Set |
| 5 | **เวลาประมวลผลมาตรฐาน (Steady-state Latency)** | **5.83 วินาทีต่อภาพ** | **533.2 มิลลิวินาทีต่อภาพ** | Primary Official Benchmark (AMD Ryzen 5, CPU, 960×960) |
...
<p align="center"><em>ที่มา: สรุปผลการทดสอบเบื้องต้นของระบบในโครงงานนี้</em></p>

> [!NOTE]
> **การศึกษาผลการประเมินเชิงลึกและสถิติเชิงอนุมาน:**
> ข้อมูลในตารางที่ 3.2 เป็นบทสรุปการทำงานเบื้องต้นเพื่อให้ผู้พัฒนาตรวจสอบความพร้อมของระบบ สำหรับรายงานผลการประเมินเชิงประจักษ์ฉบับสมบูรณ์ การศึกษาเชิงตัดทอนแบบก้าวหน้า 9 ขั้นตอน (9-Stage Progressive Ablation Study) การแจกแจงโปรโตคอลการวัดเวลา Latency รายละเอียดตัวชี้วัดระดับคลาส การวิเคราะห์ความไม่แน่นอนทางสถิติ (Wilson Score 95% CI) การตอบคำถามการวิจัย (RQ1–RQ5) และตารางสังเคราะห์ข้ออ้างและหลักฐานเชิงประจักษ์ (Claim & Evidence Matrix) ได้รับการรวบรวมและนำเสนออย่างละเอียดใน **บทที่ 4**
...
| **Exact Reading Accuracy** | **67.5% (81/120)**<br>[Wilson 95% CI: 58.7%, 75.2%] | **100.0% (7/7)**<br>[Wilson 95% CI: 64.6%, 100.0%] | ตัวชี้วัดความถูกต้องหลัก: สัดส่วนภาพที่อ่านตัวเลขถูกต้องตรงเฉลยครบทุกหลัก 100% |
...
| **เวลาประมวลผลมาตรฐาน (Steady-state Latency)** | **533.2 ms ต่อภาพ** | **5,828.8 ms ต่อภาพ** (12 สมมติฐาน) | Primary Official Benchmark (AMD Ryzen 5, CPU, 960×960) |
...
| **M7** | **+ Full Integrated Pipeline (SigLIP2 ตัวคัดกรองภาพนำเข้าเบื้องต้นแบบ Zero-shot + Full Guards)** | 100.0% [64.6%, 100.0%] | **100.0% [64.6%, 100.0%]** | 7/7 | 0.861 | 5,828.8 ms | บูรณาการท่อประมวลผล (Pipeline) เต็มรูปแบบ เพิ่มตัวคัดกรองภาพนำเข้าเพื่อปฏิเสธภาพที่ไม่ใช่มิเตอร์น้ำ |
...
##### 4.1.4.1 โปรโตคอลการวัดเวลาและการจำแนกสภาวะการทดสอบ

&emsp;&emsp;&emsp;&emsp;เพื่อป้องกันความสับสนระหว่างตัวเลขเวลาประมวลผลเฉลี่ยที่ปรากฏในเอกสาร โครงงานได้กำหนด **เวลาประมวลผลมาตรฐานหลักในสภาวะคงที่ (Primary Official Benchmark): 5.83 วินาทีต่อภาพ (AMD Ryzen 5 8645HS, CPU, 960×960, 12 สมมติฐาน)** โดยจำแนกสภาวะการวัดออกเป็น 3 โปรโตคอลเพื่อความโปร่งใสทางวิศวกรรมดังนี้:

###### 4.1.4.1.1 โปรโตคอล ก (Initial Run with Cold-start Overhead — เฉลี่ย 10.3 วินาทีต่อภาพ)

&emsp;&emsp;&emsp;&emsp;1) *ค่าที่วัดได้จริง:* เวลารวมทั้ง 7 ภาพเท่ากับ 72,339 มิลลิวินาที ส่งผลให้เวลาเฉลี่ยเท่ากับ 10,334 ms (10.33 วินาทีต่อภาพ) ตามข้อมูลดิบในตารางที่ 4.2<br>
...
&emsp;&emsp;&emsp;&emsp;1) *ค่าที่วัดได้จริง:* ค่าเฉลี่ย 8.34 วินาทีต่อภาพ (ช่วงเวลาต่ำสุด 4.9 วินาที, เฉลี่ย 8.3 วินาที, สูงสุด 26.7 วินาที) ตามที่รายงานในตารางที่ 4.1<br>
...
###### 4.1.4.1.3 โปรโตคอล ค (Steady-State Warm Run / Controlled Ablation — เฉลี่ย 5.8 วินาทีต่อภาพ)

&emsp;&emsp;&emsp;&emsp;1) *ค่าที่วัดได้จริง:* ค่าเฉลี่ย 5,828.8 ms (ประมาณ 5.8 วินาทีต่อภาพ) ในขั้นตอน M7 ของตารางที่ 4.5 และสอดคล้องตรงกันกับค่าเฉลี่ยเฉพาะภาพที่ 2 ถึง 7 ของตารางที่ 4.2 (34,942 ms / 6 ภาพ = 5,823.7 ms หรือ 5.82 วินาทีต่อภาพ)<br>
...
<p align="center"><strong>ตารางที่ 4.6: ผลการประเมินประสิทธิภาพตัวตรวจจับวัตถุ YOLO26m แยกตามคลาสตัวเลขบนชุดทดสอบอิสระ (Test Split, n=194)</strong></p>
...
&emsp;&emsp;&emsp;&emsp;ดังนั้น โครงงานนี้จึงกำหนดข้อสรุปอย่างระมัดระวัง โดยระบุว่าความแม่นยำ 100.0% เป็นผลการทำงานเฉพาะบนชุดตัวอย่างสาธิต $N=7$ เท่านั้น และนำผลการประเมินประสิทธิภาพตัวตรวจจับวัตถุบนชุดทดสอบที่กันออกจากการฝึก $N=194$ (ตารางที่ 4.4) มาเป็นหลักฐานสนับสนุนความสามารถในการตรวจจับร่วมด้วย
...
| 12 hypotheses ช่วยให้ความแม่นยำสูงกว่า Single-pass | Empirical | ผลทดลอง Progressive Ablation: M0 (42.9%) vs M2 (71.4%) vs M4 (100.0%) (ดูตารางที่ 4.7 ในหัวข้อ 4.1.3) | Supported |
| Safety Guards ช่วยยกระดับความแม่นยำ | Empirical | M3 (`is_vertical`) เพิ่มความแม่นยำ +14.3%, M4 (`red_ratio`) เพิ่มความแม่นยำ +14.3% (ดูตารางที่ 4.7 ในหัวข้อ 4.1.3) | Supported |
| YOLO26m มีประสิทธิภาพการตรวจจับสูงบนชุดทดสอบ | Empirical | Precision, Recall, mAP@50, mAP@50-95 บนชุดทดสอบอิสระ 194 ภาพ (ดูตารางที่ 4.8 ในหัวข้อ 4.1.5) | Supported |
| เวลาประมวลผลของระบบบน CPU | Empirical | CPU Benchmark บน AMD Ryzen 5 8645HS: M0=421.1ms, M1=1,710.3ms, M7=5,828.8ms (ดูหัวข้อ 4.1.4) | Supported |
...
&emsp;&emsp;&emsp;&emsp;บรรณานุกรมจัดรูปแบบตาม APA 7th edition ตรวจสอบข้อมูลจาก Google Scholar เรียงตามลำดับตัวอักษรผู้แต่งครบถ้วนระดับ 1:1 กับเนื้อหาในเอกสารคู่มือ
