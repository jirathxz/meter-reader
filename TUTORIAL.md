...
&emsp;&emsp;&emsp;&emsp;ดังภาพที่ 1.3 แสดงหลักการแปลงพิกัดย้อนกลับเมื่อภาพถูกหมุน 90 องศา เพื่อให้ตำแหน่งกล่องที่ตรวจพบบนภาพหมุนสามารถแสดงผลบนภาพต้นฉบับได้อย่างถูกต้อง

```mermaid
flowchart LR
  A["🖼️ ภาพต้นฉบับ<br/>(กว้าง W, สูง H)"] -->|"หมุนขวา 90°"| B["🔄 ภาพที่หมุนแล้ว<br/>(กว้าง H, สูง W)"]
  B -->|"ตรวจจับกล่องตัวเลขพิกัด x, y"| C["🎯 กล่องตัวเลขในภาพหมุน"]
...
```text
[ ภาพต้นฉบับ (กว้าง W, สูง H) ] ──(หมุนขวา 90°)──> [ ภาพที่หมุนแล้ว (กว้าง H, สูง W) ] ──(ตรวจจับตัวเลข x, y)──> [ กล่องตัวเลขในภาพหมุน ] ──(remap_point)──> [ กล่องตัวเลขบนภาพต้นฉบับเดิม ]
```
</details>

<p align="center"><strong>ภาพที่ 1.3: แผนผังจำลองการแปลงพิกัดย้อนกลับเมื่อหมุนภาพ 90 องศา</strong></p>
...
&emsp;&emsp;&emsp;&emsp;**แนวคิดการแก้ปัญหา:** ระบบจึงใช้วิธีการทดสอบเชิงครอบคลุมครบ **12 รูปแบบการทดสอบ (4 ทิศทาง × 3 รูปแบบการปรับแสง)** แล้วประเมินให้คะแนนเพื่อคัดเลือกรูปแบบที่ให้ผลการตรวจจับชัดเจนและน่าเชื่อถือสูงสุด ดังภาพที่ 1.4

```mermaid
flowchart LR
  subgraph Phase1 ["ระยะที่ 1: การสร้างและประมวลผล 12 สมมติฐาน"]
    A["📷 ภาพถ่ายต้นฉบับ"] --> B["🔄 สร้าง 12 สมมติฐาน<br/>(4 ทิศ × 3 ฟิลเตอร์)"]
    B --> C["⚙️ ประมวลผลย่อยแต่ละภาพ<br/>หมุน → ปรับแสง → YOLO → IoU"]
  end

  subgraph Phase2 ["ระยะที่ 2: การคัดกรองเรขาคณิตและคำนวณคะแนน"]
    C --> D["📐 ตรวจกรองแถวตัวเลข<br/>แนวนอน (is_vertical)<br/>และมี 4–9 หลัก"]
    D --> E["⭐ คำนวณคะแนน (Score)<br/>Conf เฉลี่ย × จำนวนหลัก<br/>(แดงขวา +5% / แดงซ้าย -50%)"]
  end

  subgraph Phase3 ["ระยะที่ 3: การตัดสินผู้ชนะและคืนพิกัด"]
    E --> F["🏆 ตัดสินคัดเลือกผู้ชนะ<br/>Margin Rule: ต้องชนะมุม 0°<br/>เกิน 0.12 จึงเปลี่ยนมุม"]
    F --> G["✅ แปลงพิกัดกล่องผู้ชนะ<br/>(remap_bbox สู่ระนาบต้นฉบับ)"]
  end
```
...
```text
[ ภาพถ่ายต้นฉบับ ] ──> [ สร้าง 12 สมมติฐาน (4 ทิศ × 3 ฟิลเตอร์) ] ──> [ ประมวลผล YOLO + IoU ] ──> [ กรองแนวนอน is_vertical (4-9 หลัก) ] ──> [ คำนวณคะแนน Score ] ──> [ ตัดสินด้วย Margin Rule > 0.12 ] ──> [ แปลงพิกัดกลับ remap_bbox ]
```
</details>
...
```mermaid
flowchart LR
  A["⚠️ ปัญหาหน้างานภาคสนาม"] --> B["🔍 สาเหตุความผิดพลาด<br/>ภาพกลับหัว / ภาพเบลอ / ตัวเลขสมมาตร"]
  B --> C["🛡️ เงื่อนไขความปลอดภัย<br/>ต้องตรวจสอบผลลัพธ์ก่อนส่งให้ผู้ใช้"]
  C --> D["✅ กลไก Safety Guards<br/>กรองผลลัพธ์และแจ้งเตือนความเสี่ยง"]
```
...
```text
[ ปัญหาหน้างานภาคสนาม ] ──> [ สาเหตุ: ภาพกลับหัว / ภาพเบลอ / ตัวเลขสมมาตร ] ──> [ เงื่อนไข: ต้องตรวจสอบผลลัพธ์ก่อนส่งให้ผู้ใช้ ] ──> [ กลไก: Safety Guards กรองผลลัพธ์และแจ้งเตือนความเสี่ยง ]
```
</details>
