...
&emsp;&emsp;&emsp;&emsp;**Sigmoid เทียบกับ Softmax — เหตุใดจึงใช้ Sigmoid:** ฟังก์ชัน **Softmax** บังคับให้ผลรวมความน่าจะเป็นของทุกคลาสเท่ากับ 1 ($p_i = e^{z_i}/\sum_j e^{z_j}$) จึงเหมาะกับงาน single-label ที่แต่ละภาพมีเพียงหนึ่งคำตอบ ในทางตรงกันข้าม **Sigmoid** คิดแยกอิสระต่อคู่ภาพ-ข้อความแต่ละคู่ด้วย $\sigma(z) = 1/(1+e^{-z})$ และ Binary Cross-Entropy ต่อคู่ ($\mathcal{L} = -\sum [y \log \sigma(z) + (1-y)\log(1-\sigma(z))]$) จึงรองรับ multi-label และไม่ต้อง normalize ทั้งแบทช์ ส่งผลให้การฝึกไม่ผูกกับขนาดแบทช์ ลดอคติจากคลาสลบจำนวนมาก และทำให้คะแนน Zero-shot มีความเสถียรยิ่งขึ้นเมื่อต้องเปรียบเทียบ 4 ข้อความพร้อมกันดังที่ใช้ในระบบนี้

&emsp;&emsp;&emsp;&emsp;**การทำ Zero-shot 4 ขั้นในระบบนี้:** กระบวนการจำแนกแบบซีโร่ช็อตดำเนินการ 4 ขั้นต่อเนื่อง ได้แก่ (1) **เข้ารหัสภาพ** เป็นเวกเตอร์ `image_emb` ด้วย ViT (2) **เข้ารหัสข้อความ** 4 ป้าย (`"water meter"`, `"electricity meter"`, `"gas meter"`, `"not a meter"`) เป็น `text_emb` (3) **คำนวณความคล้าย** ด้วย dot product ปรับด้วยอุณหภูมิ ($z = \text{dot}(image\_emb, text\_emb)/T + b$) ได้ `logits_per_image` และ (4) **แปลงเป็นความน่าจะเป็น** ด้วย Sigmoid ต่อคู่ หรือ Softmax เมื่อต้องการบังคับ mutually exclusive ($p_i = e^{z_i}/\sum_j e^{z_j}$) แล้วเลือกป้ายที่ได้คะแนนสูงสุด หากเป็น `"water meter"` และความเชื่อมั่น ≥ 0.50 ระบบจะถือว่า `verified=True` โดยไม่ต้องฝึกฝนใหม่แต่อย่างใด
...
> เมื่อจบหัวข้อนี้ ผู้อ่านจะสามารถอธิบายได้ว่า SigLIP2 ใช้ Sigmoid เพื่อเปรียบเทียบภาพกับข้อความแต่ละคู่แยกกัน และเข้าใจขั้นตอน 4 ขั้นของ Zero-shot ที่ทำให้ระบบคัดกรองมาตรวัดน้ำได้โดยไม่ต้องฝึกใหม่
...
&emsp;&emsp;&emsp;&emsp;**หมายเหตุ Sigmoid เทียบ Softmax ในรหัสต้นฉบับ:** แม้ SigLIP2 จะฝึกด้วย Sigmoid Loss แบบคู่ แต่เมื่อนำมาใช้คัดกรอง 4 ป้ายที่แยกจากกันอย่างสิ้นเชิง (mutually exclusive) รหัสต้นฉบับจึงประยุกต์ใช้ `torch.softmax` บน `logits_per_image` เพื่อบังคับให้ผลรวมความน่าจะเป็นเท่ากับ 1 และเลือกป้ายที่สูงสุดได้อย่างชัดเจน หากต้องการรองรับหลายป้ายพร้อมกัน สามารถเปลี่ยนเป็น `torch.sigmoid` ต่อคู่และใช้เกณฑ์ `METER_VERIFY_CONF = 0.50` ต่อป้ายได้โดยตรง ซึ่งให้ผลสอดคล้องกันในกรณี 4 ป้ายนี้ เนื่องจาก Sigmoid ที่ 0.50 เทียบเท่ากับการตัดสินแบบไบนารี (Zhai et al., 2023)
