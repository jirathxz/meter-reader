...
1.6.3.5 **การวัดระยะความคล้ายคลึง:** คำนวณผลคูณจุด (Dot Product) ระหว่างเวกเตอร์ภาพกับเวกเตอร์ข้อความแต่ละชุด ปรับสเกลด้วยพารามิเตอร์อุณหภูมิ $T$ เพื่อสร้างค่า Logits
1.6.3.6 **การตัดสินผลลัพธ์:** คำนวณความน่าจะเป็นและเลือกข้อความที่ได้คะแนนสูงสุด หากคลาส `"water meter"` ได้คะแนนความเชื่อมั่นผ่านเกณฑ์ $\ge 0.50$ ระบบจะอนุมัติให้ภาพเข้าสู่ขั้นตอนการตรวจจับตัวเลขต่อไป
...
1.6.4.2 **LAB Color Space:** ปริภูมิสีที่แยกช่องความสว่าง $L$ (Lightness) ออกจากช่องสี $A$ (แกนเขียว-แดง) และ $B$ (แกนน้ำเงิน-เหลือง) อย่างเด็ดขาด เหมาะสำหรับการปรับแต่งคอนทราสต์แสงโดยไม่ทำให้โทนสีของภาพผิดเพี้ยน
1.6.4.3 **YCrCb Color Space:** ปริภูมิสีที่แยกช่องความสว่าง $Y$ (Luminance) ออกจากช่องสัญญาณผลต่างสี $Cr$ และ $Cb$ นิยมใช้ในการกระจายความสว่างทั่วทั้งภาพ
1.6.4.4 **HSV (Hue, Saturation, Value):** ปริภูมิสีที่แยกค่าเนื้อสี ($H$), ความอิ่มตัวของสี ($S$), และความสว่าง ($V$) ออกจากกัน ช่วยให้สามารถตรวจจับวัตถุที่มีสีเฉพาะเจาะจง เช่น หลักทศนิยมสีแดง ได้อย่างแม่นยำแม้ในสภาพแสงที่มืดหรือสว่างต่างกัน

&emsp;&emsp;&emsp;&emsp;**2. ทฤษฎีการปรับปรุงคอนทราสต์ของภาพ:**
1.6.4.5 **CLAHE (Contrast Limited Adaptive Histogram Equalization):** การปรับปรุงฮิสโตแกรมแบบปรับตัวเฉพาะจุด โดยแบ่งภาพออกเป็นกริดย่อยขนาด $8 \times 8$ บล็อก แล้วกระจายความสว่างเฉพาะภายในแต่ละบล็อก พร้อมทั้งจำกัดระดับคอนทราสต์สูงสุด (Clip Limit = 2.0) เพื่อป้องกันการขยายสัญญาณรบกวน (Noise) ในบริเวณที่มืดหรือสว่างเกินไป เทคนิคนี้ดำเนินการบนช่อง $L$ ในระบบสี LAB
1.6.4.6 **Histogram Equalization (HistEq):** การปรับสมดุลฮิสโตแกรมแบบครอบคลุมทั้งภาพ (Global Histogram Equalization) โดยปรับฟังก์ชันการแจกแจงสะสมของความสว่างบนช่อง $Y$ ในระบบสี YCrCb ให้มีความสม่ำเสมอทั่วทั้งภาพ เหมาะสำหรับภาพถ่ายที่มืดสลัวทั่วทั้งเฟรม
...
1.7.4.1 หมุนขวา 90°: แกนสลับพิกัด $x_{\text{orig}} = y_{\text{rot}}, \quad y_{\text{orig}} = H - x_{\text{rot}}$
1.7.4.2 กลับหัว 180°: พิกัดย้อนกลับ $x_{\text{orig}} = W - x_{\text{rot}}, \quad y_{\text{orig}} = H - y_{\text{rot}}$
1.7.4.3 หมุนซ้าย 270°: แกนสลับพิกัด $x_{\text{orig}} = W - y_{\text{rot}}, \quad y_{\text{orig}} = x_{\text{rot}}$
...
1.12.2.2 พิกัดกล่อง `bbox [x1, y1, x2, y2]` ที่ตรวจพบจะอยู่ใน **ระบบพิกัดของภาพที่ผ่านการหมุน**
...
1.12.2.4 **หมุน 90° (ตามเข็ม):** แกนสลับกัน จุด $(x, y)$ บนภาพหมุน จะตรงกับจุด $(y, H - x)$ บนภาพเดิม
1.12.2.5 **หมุน 180° (กลับหัว):** จุด $(x, y)$ จะตรงกับจุด $(W - x, H - y)$ บนภาพเดิม
1.12.2.6 **หมุน 270° (ทวนเข็ม):** แกนสลับกัน จุด $(x, y)$ จะตรงกับจุด $(W - y, x)$ บนภาพเดิม
...
1.12.4.13 **`eval_orientation(bgr_img, angle, prep)` — ประเมินคะแนนในแต่ละรูปแบบ:**
 ...
 1.12.4.15 **วิธีการทำงาน:** หมุนภาพและปรับคอนทราสต์ตามที่กำหนด → ตรวจจับตัวเลขด้วย YOLO → กำจัดกล่องที่ซ้ำซ้อน (`dedup_detections`) → ตรวจสอบการเรียงตัวแนวนอนและจำนวนหลัก → คำนวณคะแนน $\text{Score} = \text{Mean Confidence} \times \text{จำนวนหลัก}$ พร้อมปรับคะแนนตามตำแหน่งหลักทศนิยมสีแดง
...
3.1.1.1 **ปัญหา:** เลขอารบิกบางตัวสมมาตรเมื่อหมุน 180° เช่น $6 \leftrightarrow 9$, $2 \leftrightarrow 5$, $0,1,8$ ทำให้ภาพคว่ำก็ยังอ่านได้แต่เป็นคนละเลข
...
4.4.5 **Bounding Box (`bbox`):** กรอบสี่เหลี่ยมพิกัด `[x1, y1, x2, y2]` ล้อมรอบตำแหน่งของตัวเลขแต่ละหลัก
...
4.4.29 **Coordinate Remapping:** การแปลงพิกัดจุดเรขาคณิต $(X, Y)$ จากภาพที่ผ่านการหมุนกลับสู่ระนาบพิกัดเดิมของภาพต้นฉบับ
...
4.4.32 **Mirror Check (Flip Guard):** กลไกตรวจสอบภาพกลับหัว 180° โดยเปรียบเทียบการสะท้อนของตัวเลขสมมาตร ($6 \leftrightarrow 9, 2 \leftrightarrow 5, 0, 1, 8$)
...
4.4.35 **Span-based Alignment ($\Delta X$ vs $\Delta Y$):** การคำนวณระยะกว้างเทียบกับระยะสูงเพื่อแยกแยะระหว่างแถวแนวนอนและคอลัมน์แนวตั้ง
