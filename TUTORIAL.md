...
โปรดศึกษาหลักการเบื้องต้นจำนวน 5 ประการดังต่อไปนี้ก่อนเริ่มดำเนินการ เพื่อให้การพัฒนาระบบเป็นไปอย่างราบรื่น:
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** โดยสรุปคือการเปรียบเทียบภาพกับคำอธิบาย 4 คำ แล้วพิจารณาว่าคำว่า water meter ได้คะแนนสูงสุดหรือไม่

**ประเด็นสำคัญที่ควรเข้าใจ:** SigLIP2 ทำหน้าที่เป็นด่านคัดกรองเบื้องต้น — หากมิใช่ภาพมาตรวัดน้ำ ระบบจะปฏิเสธก่อนเข้าสู่แบบจำลองหลัก

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** โปรดพิจารณากลไก Lazy Loading ด้วย `_siglip` และการประยุกต์ใช้ `torch.softmax` บน `logits_per_image` เพื่อค้นหาคำที่มีคะแนนสูงสุด
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** สรุปคือ “จัดภาพให้ตรงและชัดก่อนให้ AI ดู” — รอบแรกจำแค่นี้พอ

**ประเด็นสำคัญที่ควรเข้าใจ:** มี 4 มุมให้ลอง และ 3 วิธีปรับแสงให้ลอง ระบบจะเลือกคู่ที่อ่านชัดที่สุด

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** `rotate_image` ใช้ `cv2.rotate` ส่วน `apply_prep` เลือกปรับช่อง L (CLAHE) หรือช่อง Y (HistEq) ตามชื่อฟิลเตอร์
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** พิกัดจากภาพหมุนยังวาดบนภาพเดิมไม่ได้ ต้องแปลงกลับก่อน

**ประเด็นสำคัญที่ควรเข้าใจ:** หมุนมุมไหน ต้องย้อนกลับมุมนั้น — งานของ `remap_point` คือจุดเดียว ส่วน `remap_bbox` คือทำกับมุมสองมุมแล้วหาขอบใหม่

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** สูตร `y, h-x` / `w-x, h-y` / `w-y, x` คือการสลับแกนตามองศา โดย `w, h` คือขนาดภาพต้นฉบับ (มุมซ้ายบนเป็น 0,0) และ `remap_bbox` แค่เรียก `remap_point` สองครั้งแล้ว `min/max` — ทั้งสองสูตรตรงกับโค้ดจริงใน `main.py:remap_point()` (บรรทัด 121–129) และ `remap_bbox()` (132–142) แบบ 1:1
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** ถ้าแถวตั้งสูงกว่ากว้าง ให้ตีว่าไม่ใช่หน้าปัดมิเตอร์

**ประเด็นสำคัญที่ควรเข้าใจ:** หน้าปัดจริงต้องกว้างกว่าสูง — ถ้าสูงกว่ากว้าง คือป้ายวันที่/ซีเรียลแนวตั้ง

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** แปลง `center_x/center_y` เป็นสัดส่วนภาพแล้วเทียบ `width_span` กับ `height_span` ด้วยเกณฑ์ `0.8`
...
**แต่ละขั้นแก้ปัญหาอะไร (ตอบคำถามนี้ก่อนดูโค้ด):**
...
**การทำงานแบ่งออกเป็น 3 ฟังก์ชันย่อยตามหน้าที่ (แต่ละฟังก์ชันแก้ปัญหาไหน):**
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** ทดสอบครบทั้ง 12 รูปแบบ ให้คะแนนด้วยความเชื่อมั่นเฉลี่ยคูณจำนวนหลักร่วมกับกฎสีแดง แล้วคัดเลือกมุมที่ชนะโดยใช้เกณฑ์ Margin เพื่อป้องกันการเปลี่ยนแปลงผลลัพธ์เนื่องจากคะแนนใกล้เคียงกัน

**ประเด็นสำคัญที่ควรเข้าใจ:** ผู้ท้าชิงทั้ง 12 รูปแบบจะได้รับการประเมิน โดยผู้ที่ผ่านการตรวจสอบแนวนอนและมีจำนวนหลัก 4–9 หลัก พร้อมทั้งได้คะแนนสูงสุดจะเป็นผู้ชนะ

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** โปรดพิจารณาว่า `red_ratio` คำนวณสัดส่วนพิกเซลสีแดงในปริภูมิสี HSV อย่างไร, `eval_orientation` รวมคะแนนและปรับลด 50% หรือเพิ่ม 5% ตามตำแหน่งสีแดงอย่างไร, และ `detect_digits_best` คัดเลือก `best_in_angle` แล้วประยุกต์ใช้ Margin 0.12 อย่างไร
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** หากภาพกลับหัว ตัวเลขที่อ่านได้จะมีลักษณะสมมาตรแบบกระจกเงา — พึงทดลองหมุนภาพ 180 องศาแล้วเปรียบเทียบผลลัพธ์

**ประเด็นสำคัญที่ควรเข้าใจ:** กลุ่มตัวเลขที่มีความสมมาตรคือ 6↔9, 2↔5, 0/1/8 — หากผลการหมุนไม่สอดคล้องกับตาราง `FLIP_MAP` พึงแจ้งเตือน

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** โปรดพิจารณาการเปรียบเทียบ `reversed(digits)` กับ `anti_dets` และกระบวนการ `cross_check_digits` ที่ใช้ SigLIP2 ตรวจสอบซ้ำเฉพาะหลักที่ YOLO มีความเชื่อมั่นต่ำ (<0.60)
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** `read_meter` ดำเนินการ 4 ขั้นตอนตามลำดับ — ตรวจสอบชนิดมาตรวัด → ตรวจจับตัวเลข → กรองแนวตั้ง → สร้างรายการแจ้งเตือน

**ประเด็นสำคัญที่ควรเข้าใจ:** การศึกษารหัสต้นฉบับตามลำดับขั้นตอนจะพบว่าทุกกรณีมี `warnings` อธิบายสาเหตุเสมอ — ในการศึกษารอบแรกพึงมุ่งเน้นที่ลำดับขั้นตอนเป็นหลัก

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** โปรดพิจารณาการสร้าง `align_ok` (ตรวจสอบความตรงของแถว), `mismatches` (การตรวจสอบซ้ำด้วย SigLIP2) และการรวบรวม `warns` ตามเงื่อนไขต่าง ๆ
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** FastAPI เปิดให้บริการ 2 เส้นทาง — `GET /api/health` สำหรับตรวจสอบสถานะ และ `POST /api/read-meter` สำหรับรับภาพ

**ประเด็นสำคัญที่ควรเข้าใจ:** การส่งภาพในรูปแบบ `UploadFile` จะได้รับผลลัพธ์กลับในรูปแบบ JSON — ในการศึกษาเบื้องต้นพึงมุ่งเน้นที่ 2 เส้นทางดังกล่าว

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** โปรดพิจารณาการตรวจสอบ `ALLOWED_TYPES`, การแปลงภาพด้วย `PIL` และการใช้งาน `run_in_threadpool` เพื่อป้องกันการบล็อก Event Loop ของเซิร์ฟเวอร์
...
**บทสรุปก่อนศึกษารหัสต้นฉบับ:** Gradio ทำหน้าที่สร้างส่วนติดต่อผู้ใช้บนเว็บสำหรับอัปโหลดภาพ เรียก API และแสดงผล — โดยไม่จำเป็นต้องเขียนรหัสเรียก API ด้วยตนเอง

**ประเด็นสำคัญที่ควรเข้าใจ:** ส่วนติดต่อผู้ใช้ประกอบด้วย 3 ส่วนหลัก — พื้นที่อัปโหลดภาพ, ปุ่มอ่านค่า, และพื้นที่แสดงผล — ในการศึกษาเบื้องต้นพึงทำความเข้าใจเพียงประเด็นดังกล่าว

**รายละเอียดเชิงเทคนิค (สำหรับศึกษารอบที่สอง) (เก็บไว้อ่านรอบสอง):** โปรดพิจารณาการแปลงภาพเป็น JPEG ผ่าน `BytesIO`, การส่งคำขอ `POST` ด้วย `httpx` และการแสดงผล `reading/conf/warn_text`
