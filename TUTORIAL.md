...
[บทที่ 1: ทฤษฎีและเทคโนโลยีที่เกี่ยวข้อง ... 4](#บทที่-1-ทฤษฎีและเทคโนโลยีที่เกี่ยวข้อง)  
	&emsp;[1.1 ภาพรวมระบบและบทบาทของเทคโนโลยี ... 4](#11-ภาพรวมระบบและบทบาทของเทคโนโลยี)  
	&emsp;[1.2 ทฤษฎีการตรวจจับวัตถุด้วย YOLO ... 4](#12-ทฤษฎีการตรวจจับวัตถุด้วย-yolo)  
		&emsp;&emsp;[1.2.1 คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ ... 4](#121-คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ)  
		&emsp;&emsp;[1.2.2 สถาปัตยกรรม YOLO26 ... 5](#122-สถาปัตยกรรม-yolo26)  
		&emsp;&emsp;[1.2.3 เหตุผลที่เลือก YOLO26 Nano ... 5](#123-เหตุผลที่เลือก-yolo26-nano)  
	&emsp;[1.3 ทฤษฎีการจำแนกภาพแบบซีโร่ช็อตด้วย SigLIP2 ... 5](#13-ทฤษฎีการจำแนกภาพแบบซีโร่ช็อตด้วย-siglip2)  
		&emsp;&emsp;[1.3.1 SigLIP2 คืออะไร ... 5](#131-siglip2-คืออะไร)  
		&emsp;&emsp;[1.3.2 Sigmoid เทียบ Softmax ... 6](#132-sigmoid-เทียบ-softmax)  
		&emsp;&emsp;[1.3.3 กระบวนการ Zero-shot 4 ขั้น ... 6](#133-กระบวนการ-zero-shot-4-ขั้น)  
	&emsp;[1.4 ทฤษฎีการประมวลผลภาพดิจิทัลด้วย OpenCV ... 6](#14-ทฤษฎีการประมวลผลภาพดิจิทัลด้วย-opencv)  
		&emsp;&emsp;[1.4.1 ระบบสีและแนวคิด Image Preprocessing ... 6](#141-ระบบสีและแนวคิด-image-preprocessing)  
		&emsp;&emsp;[1.4.2 เทคนิค CLAHE และ Histogram Equalization ... 7](#142-เทคนิค-clahe-และ-histogram-equalization)  
	&emsp;[1.5 ทฤษฎีการให้บริการเว็บด้วย FastAPI ... 7](#15-ทฤษฎีการให้บริการเว็บด้วย-fastapi)  
	&emsp;[1.6 ทฤษฎีส่วนติดต่อผู้ใช้ด้วย Gradio ... 7](#16-ทฤษฎีส่วนติดต่อผู้ใช้ด้วย-gradio)  
	&emsp;[1.7 ทฤษฎีการจัดการสภาพแวดล้อมด้วย uv ... 7](#17-ทฤษฎีการจัดการสภาพแวดล้อมด้วย-uv)  
	&emsp;[1.8 สรุปความสัมพันธ์ของเทคโนโลยี ... 8](#18-สรุปความสัมพันธ์ของเทคโนโลยี)  
[บทที่ 2: การพัฒนาระบบอ่านค่ามาตรวัดน้ำเชิงปฏิบัติ ... 9](#บทที่-2-การพัฒนาระบบอ่านค่ามาตรวัดน้ำเชิงปฏิบัติ)  
	&emsp;[2.1 ภาพรวมกระบวนการทำงานเชิงปฏิบัติ ... 9](#21-ภาพรวมกระบวนการทำงานเชิงปฏิบัติ)  
	&emsp;[2.2 การทดสอบไลบรารีเบื้องต้นก่อนลงมือจริง ... 9](#22-การทดสอบไลบรารีเบื้องต้นก่อนลงมือจริง)  
		&emsp;&emsp;[2.2.1 ทดสอบ SigLIP2 สำหรับ Image Classification ... 9](#221-ทดสอบ-siglip2-สำหรับ-image-classification)  
		&emsp;&emsp;[2.2.2 ทดสอบ YOLO สำหรับ Object Detection ... 9](#222-ทดสอบ-yolo-สำหรับ-object-detection)  
		&emsp;&emsp;[2.2.3 ทดสอบ OpenCV สำหรับ Image Processing ... 10](#223-ทดสอบ-opencv-สำหรับ-image-processing)  
		&emsp;&emsp;[2.2.4 ทดสอบ FastAPI สำหรับสร้าง API ... 10](#224-ทดสอบ-fastapi-สำหรับสร้าง-api)  
		&emsp;&emsp;[2.2.5 ทดสอบ Gradio สำหรับสร้าง Web UI ... 10](#225-ทดสอบ-gradio-สำหรับสร้าง-web-ui)  
	&emsp;[2.3 ขั้นตอนที่ 1: การรับภาพถ่ายนำเข้า ... 10](#23-ขั้นตอนที่-1-การรับภาพถ่ายนำเข้า)  
	&emsp;[2.4 ขั้นตอนที่ 2: การเตรียมชุดข้อมูลและการฝึกแบบจำลอง YOLO ... 11](#24-ขั้นตอนที่-2-การเตรียมชุดข้อมูลและการฝึกแบบจำลอง-yolo)  
		&emsp;&emsp;[2.4.1 การเตรียมชุดข้อมูลจาก Roboflow ... 11](#241-การเตรียมชุดข้อมูลจาก-roboflow)  
		&emsp;&emsp;[2.4.2 การฝึกแบบจำลอง YOLO26 ... 11](#242-การฝึกแบบจำลอง-yolo26)  
	&emsp;[2.5 ขั้นตอนที่ 3: การจัดเตรียมสภาพแวดล้อมและโครงสร้างไฟล์ด้วย uv ... 11](#25-ขั้นตอนที่-3-การจัดเตรียมสภาพแวดล้อมและโครงสร้างไฟล์ด้วย-uv)  
	&emsp;[2.6 ขั้นตอนที่ 4: การคัดกรองภาพด้วย SigLIP2 ... 12](#26-ขั้นตอนที่-4-การคัดกรองภาพด้วย-siglip2)  
	&emsp;[2.7 ขั้นตอนที่ 5: การปรับปรุงคุณภาพภาพและการจัดการทิศทาง ... 12](#27-ขั้นตอนที่-5-การปรับปรุงคุณภาพภาพและการจัดการทิศทาง)  
	&emsp;[2.8 ขั้นตอนที่ 6: การแปลงพิกัดและการกรองข้อมูล ... 12](#28-ขั้นตอนที่-6-การแปลงพิกัดและการกรองข้อมูล)  
		&emsp;&emsp;[2.8.1 การหมุนภาพและการปรับปรุงคุณภาพภาพ ... 12](#281-การหมุนภาพและการปรับปรุงคุณภาพภาพ)  
		&emsp;&emsp;[2.8.2 การแปลงพิกัดจุดเดี่ยวและกล่องขอบเขต ... 13](#282-การแปลงพิกัดจุดเดี่ยวและกล่องขอบเขต)  
		&emsp;&emsp;[2.8.3 การกรองแถวแนวตั้งและการตรวจสอบสีแดง ... 13](#283-การกรองแถวแนวตั้งและการตรวจสอบสีแดง)  
	&emsp;[2.9 ขั้นตอนที่ 7: การค้นหาทิศทางและฟิลเตอร์ที่เหมาะสม ... 13](#29-ขั้นตอนที่-7-การค้นหาทิศทางและฟิลเตอร์ที่เหมาะสม)  
	&emsp;[2.10 ขั้นตอนที่ 8: กลไกความปลอดภัยและการบอกค่าตัวเลข ... 14](#210-ขั้นตอนที่-8-กลไกความปลอดภัยและการบอกค่าตัวเลข)  
	&emsp;[2.11 การให้บริการระบบผ่าน FastAPI ... 14](#211-การให้บริการระบบผ่าน-fastapi)  
	&emsp;[2.12 การพัฒนาส่วนติดต่อผู้ใช้ด้วย Gradio ... 15](#212-การพัฒนาส่วนติดต่อผู้ใช้ด้วย-gradio)  
	&emsp;[2.13 การทดสอบระบบและการแก้ไขปัญหา ... 15](#213-การทดสอบระบบและการแก้ไขปัญหา)  
[อภิธานศัพท์ (Glossary) ... 16](#อภิธานศัพท์-glossary)  
	&emsp;[1. หมวดปัญญาประดิษฐ์และการเรียนรู้เชิงลึก ... 16](#1-หมวดปัญญาประดิษฐ์และการเรียนรู้เชิงลึก-ai--deep-learning)  
	&emsp;[2. หมวดการประมวลผลภาพดิจิทัล ... 16](#2-หมวดการประมวลผลภาพดิจิทัล-digital-image-processing--opencv)  
	&emsp;[3. หมวดสถาปัตยกรรมซอฟต์แวร์และเว็บ ... 16](#3-หมวดสถาปัตยกรรมซอฟต์แวร์และเว็บ-software-architecture--web)  
...
---
# บทที่ 1: ทฤษฎีและเทคโนโลยีที่เกี่ยวข้อง

---

### 1.1 ภาพรวมระบบและบทบาทของเทคโนโลยี

&emsp;&emsp;&emsp;&emsp;ระบบอ่านค่ามาตรวัดน้ำอัตโนมัติทำหน้าที่รับภาพถ่ายจากผู้ใช้งาน ประมวลผลภาพด้วยปัญญาประดิษฐ์ และส่งออกตัวเลขพร้อมค่าความเชื่อมั่น ระบบนี้ประยุกต์ใช้เทคโนโลยีหลัก 5 ชนิด ได้แก่ YOLO สำหรับตรวจจับตัวเลข SigLIP2 สำหรับคัดกรองประเภทภาพ OpenCV สำหรับปรับปรุงคุณภาพภาพ FastAPI สำหรับให้บริการผ่านเครือข่าย และ Gradio สำหรับส่วนติดต่อผู้ใช้บนเว็บ ไลบรารีเสริม `uv` ทำหน้าที่จัดการสภาพแวดล้อมและติดตั้งชุดคำสั่งให้ทำงานได้ซ้ำบนทุกเครื่อง

&emsp;&emsp;&emsp;&emsp;ผู้พัฒนาออกแบบกระบวนการทำงานเป็นลำดับขั้นที่ชัดเจน โดยเริ่มจากการรับภาพถ่าย นำภาพไปคัดกรองด้วย SigLIP2 ทดลองหมุนและปรับแสงด้วย OpenCV ตรวจจับตัวเลขด้วย YOLO คัดกรองผลลัพธ์ด้วยกลไกความปลอดภัย และส่งออกผลลัพธ์ผ่าน FastAPI ให้ Gradio แสดงผล ภาพที่ 1.1 แสดงการไหลของข้อมูลดังกล่าว

```mermaid
%% 4:3 compact — ภาพรวมเทคโนโลยี
flowchart TD
  subgraph Row1 [ ]
  direction LR
  A[" Input"] --> B["1. SigLIP2"] --> C["2. Preprocessing<br/>OpenCV 4×3"]
  end
  subgraph Row2 [ ]
  direction LR
...
<p align="center"><strong>ภาพที่ 1.1 ภาพรวมบทบาทของเทคโนโลยีในระบบ Water Meter OCR</strong></p>

#### 1.1.1 ปัญหาและความท้าทายของภาพถ่ายมาตรวัดน้ำ

&emsp;&emsp;&emsp;&emsp;ภาพถ่ายมาตรวัดน้ำจากการปฏิบัติงานจริงมีความหลากหลายและปัจจัยรบกวนแตกต่างจากห้องปฏิบัติการ ระบบจึงพบปัญหาสำคัญ 4 ประการที่เทคโนโลยีแต่ละชนิดต้องแก้ไข

#### 1.1.0.1 การถ่ายภาพเอียงหรือกลับหัว

&emsp;&emsp;&emsp;&emsp;ผู้ใช้งานถ่ายภาพด้วยอุปกรณ์เคลื่อนที่ในมุมเอียง 90 องศาหรือกลับหัว 180 องศา ระบบจึงต้องทดสอบการหมุน 4 ทิศทาง

#### 1.1.0.2 แสงสะท้อนและตัวเลขเลือนราง

&emsp;&emsp;&emsp;&emsp;กระจกหน้าปัดมีแสงสะท้อน คราบน้ำ หรือสภาพแสงน้อย ระบบจึงต้องปรับคุณภาพภาพด้วย OpenCV ก่อนตรวจจับ

#### 1.1.0.3 ป้ายข้อมูลอื่นบนตัวเรือน

&emsp;&emsp;&emsp;&emsp;ตัวถังมาตรวัดน้ำมีตัวเลขวันที่ผลิตหรือหมายเลขซีเรียลพิมพ์แนวตั้ง ระบบจึงต้องกรองแถวแนวตั้งออก

#### 1.1.0.4 ตัวเลขที่มีลักษณะสมมาตรเมื่อกลับหัว

&emsp;&emsp;&emsp;&emsp;ตัวเลขกลุ่ม $6 \leftrightarrow 9$, $2 \leftrightarrow 5$, $0,1,8$ มีลักษณะสมมาตรเมื่อหมุน 180 องศา ระบบจึงต้องมีกลไกตรวจสอบภาพกลับหัว

---

### 1.2 ทฤษฎีการตรวจจับวัตถุด้วย YOLO

&emsp;&emsp;&emsp;&emsp;YOLO ทำหน้าที่ตรวจจับและระบุตำแหน่งตัวเลขบนหน้าปัดมาตรวัดน้ำ ระบบเลือกใช้ YOLO เพราะ YOLO มองภาพครั้งเดียวแล้วตอบทันที จึงให้ความเร็วระดับเวลาจริงและเหมาะกับงานที่ต้องประมวลผลต่อเนื่อง

#### 1.2.1 คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ

&emsp;&emsp;&emsp;&emsp;**คอมพิวเตอร์วิทัศน์ (Computer Vision)** ทำหน้าที่สอนคอมพิวเตอร์ให้วิเคราะห์ภาพถ่ายได้เช่นเดียวกับสายตามนุษย์ **การตรวจจับวัตถุ (Object Detection)** ทำหน้าที่ระบุตำแหน่งและจำแนกประเภทวัตถุในภาพพร้อมกัน ส่วน **OCR (Optical Character Recognition)** ทำหน้าที่แปลงบริเวณตัวเลขในภาพให้เป็นข้อความดิจิทัลที่นำไปจัดเก็บได้ (Zou et al., 2023)

&emsp;&emsp;&emsp;&emsp;YOLO แบ่งภาพเป็นกริด ทำนายกรอบและประเภทพร้อมกันในรอบเดียว หลักการนี้แตกต่างจากแบบจำลองสองขั้นตอนอย่าง Faster R-CNN ที่ต้องเสนอพื้นที่ก่อนแล้วจึงจำแนก YOLO จึงให้ความเร็วสูงและเหมาะกับงานมิเตอร์น้ำที่ต้องรันต่อเนื่อง (Jocher et al., 2026)

#### 1.2.2 สถาปัตยกรรม YOLO26

&emsp;&emsp;&emsp;&emsp;YOLO26 เป็นรุ่นต่อยอดจาก YOLOv8/YOLO11 ที่ปรับเป็น End-to-End Vision Model แบบ Anchor-free (Ultralytics, 2025–2026) สถาปัตยกรรมนี้ประกอบด้วย 3 ส่วนหลัก

##### 1.2.2.1 Backbone (CSPDarknet)

&emsp;&emsp;&emsp;&emsp;Backbone ทำหน้าที่สกัดคุณลักษณะภาพหลายระดับจากภาพถ่ายนำเข้า

##### 1.2.2.2 Neck (PAN-FPN)

&emsp;&emsp;&emsp;&emsp;Neck ทำหน้าที่รวมคุณลักษณะระดับเล็กและระดับใหญ่เพื่อให้ระบบมองเห็นทั้งตัวเลขขนาดเล็กและขนาดใหญ่

##### 1.2.2.3 Head แบบ Anchor-free

&emsp;&emsp;&emsp;&emsp;Head ทำหน้าที่ทำนาย `bbox` + `confidence` + `class` โดยตรงโดยไม่ต้องพึ่งกล่องต้นแบบ ระบบจึงลดการปรับจูนและเพิ่มความแม่นยำกับวัตถุขนาดเล็ก

#### 1.2.3 เหตุผลที่เลือก YOLO26 Nano

&emsp;&emsp;&emsp;&emsp;โครงงานเลือก YOLO26 Nano (~2.6 ล้านพารามิเตอร์) เพราะ Nano ให้สมดุลระหว่างความแม่นยำและความเร็วที่เหมาะกับข้อจำกัดของงานมิเตอร์น้ำ ตารางที่ 1.1 เปรียบเทียบตัวเลือก
...
| Tesseract OCR | — | ต่ำ | ~100 ms | ใช้ง่าย | ล้มเหลวเมื่อภาพเอียง/เบลอ — ต้องครอปเลขมาก่อน |

&emsp;&emsp;&emsp;&emsp;YOLO26 Nano ใช้ VRAM น้อยกว่า 2 GB ระบบจึงฝึกด้วย `batch=64` บน Tesla T4 ได้ และประมวลผลบน CPU ภายใน 1–3 วินาทีต่อภาพ ความแม่นยำของ Nano เพียงพอสำหรับเลข 0–9 ที่รูปแบบจำกัด และระบบชดเชยด้วยกลไก 12 รูปแบบ (หมุน 4 ทิศ × ปรับแสง 3 แบบ)

##### 1.2.3.1 เหตุผล 5 ประการที่ระบบเลือก YOLO26

&emsp;&emsp;&emsp;&emsp;ระบบเลือก YOLO26 ด้วยเหตุผล 5 ประการ ประการที่หนึ่ง เลขมิเตอร์มีหลายหลักเรียงแนวนอน 5–7 หลัก YOLO เห็นวัตถุหลายชิ้นพร้อมกันด้วยกริดเดียว ประการที่สอง ภาพมีเบลอ/แสงสะท้อน/เงา YOLO ทนการบิดเบือนเพราะฝึกด้วย Mosaic/Mixup/HSV ประการที่สาม ระบบต้องการความเร็วเวลาจริง 50–150 มิลลิวินาทีต่อภาพ ประการที่สี่ ระบบต้องการค่าความมั่นใจต่อหลักเพื่อส่งให้ `flip_guard` ตัดสินใจ ประการที่ห้า YOLO หาเลขจากภาพเต็มได้ทันทีโดยไม่ต้องครอปเลขมาก่อน ต่างจาก Tesseract ที่ต้องตัดภาพให้ตรงก่อน

&emsp;&emsp;&emsp;&emsp;คำศัพท์สำคัญ 3 รายการที่ระบบใช้ได้แก่ **Bounding Box (`bbox`)** กรอบ `[x1, y1, x2, y2]` ที่ระบุตำแหน่งตัวเลข **Confidence Score** ค่า 0.00–1.00 ที่บอกความมั่นใจ และ **Class** ประเภทเลข 0–9 ที่ระบบตรวจพบ

---

### 1.3 ทฤษฎีการจำแนกภาพแบบซีโร่ช็อตด้วย SigLIP2

&emsp;&emsp;&emsp;&emsp;SigLIP2 ทำหน้าที่คัดกรองภาพว่าเป็นมาตรวัดน้ำหรือไม่ก่อนส่งให้ YOLO ระบบใช้ SigLIP2 เพราะ SigLIP2 จำแนกภาพตามคำอธิบายภาษาธรรมชาติได้โดยไม่ต้องฝึกใหม่ จึงเหมาะเป็นด่านคัดกรองเบื้องต้น

#### 1.3.1 SigLIP2 คืออะไร

&emsp;&emsp;&emsp;&emsp;SigLIP2 (Sigmoid Loss for Language Image Pre-training 2) เป็นแบบจำลอง Vision-Language รุ่นต่อยอดจาก CLIP และ SigLIP ที่ใช้ Vision Encoder (ViT-B/16) และ Text Encoder (Transformer) แยกกัน (Zhai et al., 2023; Tschannen et al., 2025) จุดต่างคือ SigLIP2 ใช้ Sigmoid Loss แบบคู่แทน Softmax จึงรองรับการฝึกแบบกระจายบนแบทช์ขนาดใหญ่และข้อมูลไม่สมดุลได้เสถียรกว่า และให้คุณลักษณะเชิงความหมายที่แม่นยำสำหรับการเปรียบเทียบภาพกับข้อความ

#### 1.3.2 Sigmoid เทียบ Softmax

&emsp;&emsp;&emsp;&emsp;Softmax ทำหน้าที่แจกคะแนนรวม 100% ให้ทุกคลาสแล้วบังคับเลือกเพียงหนึ่งคำตอบ ระบบจึงเหมาะเมื่อภาพมีคำตอบเดียว Sigmoid ทำหน้าที่ให้คะแนนถูก/ผิดแยกคู่ต่อคู่ แต่ละคู่ภาพ-ข้อความได้คะแนน 0–1 อิสระจากข้ออื่น ระบบจึงรองรับกรณีที่ภาพตรงกับหลายคำอธิบาย SigLIP2 เลือก Sigmoid เพราะฝึกแบบคู่ภาพ-ข้อความทีละคู่ ไม่ต้องรวมคะแนนทั้งแบทช์ จึงฝึกบนข้อมูลขนาดใหญ่และไม่สมดุลได้เสถียรกว่า

&emsp;&emsp;&emsp;&emsp;**ตารางที่ 1.2 เปรียบเทียบ Softmax กับ Sigmoid**
...
&emsp;&emsp;&emsp;&emsp;สูตรอย่างง่าย Sigmoid แปลงค่าดิบ $z$ เป็นความน่าจะเป็นด้วย $\sigma(z)=1/(1+e^{-z})$ หาก $z=2.0$ ระบบได้ $0.88$ (มั่นใจสูง) หาก $z=0$ ระบบได้ $0.50$ (ก้ำกึ่ง) หาก $z=-1.0$ ระบบได้ $0.27$ (ไม่ตรง) ส่วน Softmax คิด $p_i=e^{z_i}/\sum_j e^{z_j}$ เพื่อบังคับให้รวมกันเป็น 1

#### 1.3.3 กระบวนการ Zero-shot 4 ขั้น

&emsp;&emsp;&emsp;&emsp;ระบบทำ Zero-shot 4 ขั้น ขั้นที่หนึ่ง ตัวเข้ารหัสภาพแปลงภาพถ่ายเป็นเวกเตอร์ `image_emb` ขั้นที่สอง ตัวเข้ารหัสข้อความแปลง 4 ป้าย (`"water meter"`, `"electricity meter"`, `"gas meter"`, `"not a meter"`) เป็น `text_emb` ขั้นที่สาม ระบบวัดความคล้ายด้วย dot product แล้วปรับด้วยอุณหภูมิได้ค่าดิบ $z$ ขั้นที่สี่ ระบบแปลง $z$ ด้วย Sigmoid หรือ Softmax แล้วเลือกป้ายที่ได้คะแนนสูงสุด หากเป็น `"water meter"` และความมั่นใจ ≥ 0.50 ระบบตัดสินว่า `verified=True`

&emsp;&emsp;&emsp;&emsp;**ตัวอย่างตัวเลขที่ระบบคำนวณ** เมื่อภาพมาตรวัดน้ำได้ค่า $z$ ดังนี้ `water meter 1.8`, `electricity meter -0.2`, `gas meter -0.8`, `not a meter -0.5` Sigmoid ให้ $0.86$ สำหรับ `"water meter"$ และ Softmax ให้ $0.71$ ซึ่งสูงสุดเช่นกัน ระบบจึงตัดสินว่า `verified=True` ตรงตามเกณฑ์ `METER_VERIFY_CONF = 0.50`

##### 1.3.3.1 Zero-shot Classification

&emsp;&emsp;&emsp;&emsp;Zero-shot Classification ทำหน้าที่จำแนกภาพตามคำอธิบายภาษาธรรมชาติโดยไม่ต้องฝึกใหม่ ระบบกำหนดข้อความเปรียบเทียบแล้วให้แบบจำลองคำนวณความน่าจะเป็นของข้อความที่สอดคล้องกับภาพมากที่สุด

##### 1.3.3.2 Sigmoid

&emsp;&emsp;&emsp;&emsp;Sigmoid ทำหน้าที่แปลง logit เป็นความน่าจะเป็นอิสระต่อคู่ 0–1 ระบบใช้ Sigmoid ใน SigLIP2 เพื่อให้แต่ละคู่ภาพ-ข้อความตัดสินใจแยกกัน

##### 1.3.3.3 SigLIP / SigLIP2

&emsp;&emsp;&emsp;&emsp;SigLIP และ SigLIP2 เป็นตระกูล Vision-Language จาก Google ที่ฝึกด้วย Sigmoid Loss แทน Softmax (Zhai et al., 2023) SigLIP2 เพิ่มความสามารถหลายภาษาและการเข้าใจเชิงตำแหน่งให้แม่นยำยิ่งขึ้น (Tschannen et al., 2025)
...
### 1.4 ทฤษฎีการประมวลผลภาพดิจิทัลด้วย OpenCV
...
#### 1.4.1 ระบบสีและแนวคิด Image Preprocessing

&emsp;&emsp;&emsp;&emsp;ระบบสี RGB/BGR ทำหน้าที่เก็บค่าสีแดง-เขียว-น้ำเงิน OpenCV เรียงช่องสีเป็น BGR เป็นค่าเริ่มต้น ระบบสี LAB แยกช่องความสว่าง L ออกจากช่องสี A, B ระบบจึงปรับคอนทราสต์ด้วย CLAHE บนช่อง L ได้โดยไม่เพี้ยนสี ส่วนระบบสี YCrCb แยกช่องความสว่าง Y ออกจาก Cr, Cb ระบบจึงทำ Histogram Equalization บนช่อง Y ได้ ส่วนระบบสี HSV แยก Hue-Saturation-Value ระบบจึงตรวจหาสีแดงของหลักทศนิยมได้แม่น

#### 1.4.2 เทคนิค CLAHE และ Histogram Equalization

&emsp;&emsp;&emsp;&emsp;CLAHE ทำหน้าที่ปรับแสงเฉพาะจุดบนช่องความสว่าง L ในระบบสี LAB เทคนิคนี้ดึงรายละเอียดในเงามืดโดยไม่ทำให้ส่วนสว่างจ้าเกินไป ระบบจึงเหมาะกับภาพที่มีเงาตกเป็นหย่อม ส่วน Histogram Equalization ทำหน้าที่เกลี่ยความสว่างทั่วทั้งภาพบนช่อง Y ในระบบสี YCrCb เทคนิคนี้เหมาะกับภาพที่มืดทั้งภาพ

##### 1.4.2.1 CLAHE

&emsp;&emsp;&emsp;&emsp;CLAHE ปรับแสงเฉพาะจุดบนช่องความสว่าง L ในระบบสี LAB เทคนิคนี้ดึงรายละเอียดในเงามืดโดยไม่ทำให้ส่วนสว่างจ้าเกินไป

##### 1.4.2.2 Histogram Equalization (HistEq)

&emsp;&emsp;&emsp;&emsp;Histogram Equalization เกลี่ยความสว่างทั่วทั้งภาพบนช่อง Y ในระบบสี YCrCb เทคนิคนี้เหมาะกับภาพที่มืดทั้งภาพ

---

### 1.5 ทฤษฎีการให้บริการเว็บด้วย FastAPI

&emsp;&emsp;&emsp;&emsp;FastAPI ทำหน้าที่สร้างเว็บเซอร์วิส REST API สำหรับให้แอปอื่นส่งภาพมาอ่านค่า ไลบรารีนี้ใช้ Python สร้าง API ประสิทธิภาพสูง มีระบบสร้างเอกสาร Swagger UI อัตโนมัติที่ `/docs` และรองรับการทำงานแบบ Asynchronous

&emsp;&emsp;&emsp;&emsp;FastAPI ทำหน้าที่รับคำขอ HTTP POST ที่ `/api/read-meter` พร้อมไฟล์ภาพ แล้วส่งผลลัพธ์กลับเป็น JSON มาตรฐาน ระบบใช้ FastAPI เพราะ FastAPI ให้ความเร็วใกล้เคียง Node.js/Go มีการตรวจสอบชนิดไฟล์อัตโนมัติ และรวม CORS, Validation, Async ไว้พร้อม

&emsp;&emsp;&emsp;&emsp;เทคนิคสำคัญของ FastAPI คือการแยกงานประมวลผลหนักไปรันบน Threadpool ด้วย `run_in_threadpool` เทคนิคนี้ป้องกันไม่ให้ Event Loop ของเซิร์ฟเวอร์ถูกบล็อกขณะรัน YOLO/SigLIP2 ระบบจึงรองรับหลายคำขอพร้อมกันได้

&emsp;&emsp;&emsp;&emsp;FastAPI ให้บริการ 2 เส้นทางหลัก เส้นทางแรก `GET /api/health` ทำหน้าที่รายงานสถานะ `device`, `yolo_loaded`, `siglip_loaded` เส้นทางที่สอง `POST /api/read-meter` ทำหน้าที่รับภาพ ประมวลผล และส่ง `reading`, `digits`, `confidence`, `warnings` กลับ

---

### 1.6 ทฤษฎีส่วนติดต่อผู้ใช้ด้วย Gradio

&emsp;&emsp;&emsp;&emsp;Gradio ทำหน้าที่สร้างหน้าเว็บสำหรับให้ผู้ใช้งานทั่วไปทดสอบอัปโหลดภาพและดูผลลัพธ์โดยไม่ต้องเรียก API เอง ไลบรารีนี้สร้าง Web UI แบบ Interactive ได้ภายในไม่กี่บรรทัด

&emsp;&emsp;&emsp;&emsp;Gradio ทำหน้าที่แสดงช่องอัปโหลดภาพ ปุ่มประมวลผล ตารางค่าตัวเลขรายหลัก และกล่องคำเตือน ส่วนหน้าเว็บสื่อสารกับ FastAPI Backend ผ่าน `httpx.post` ที่ `http://127.0.0.1:8000/api/read-meter`

&emsp;&emsp;&emsp;&emsp;ระบบใช้ Gradio เพราะ Gradio ติดตั้งง่าย รันด้วย `demo.launch(server_port=7860)` แล้วเปิดเบราว์เซอร์ที่ `http://127.0.0.1:7860` ได้ทันที และเชื่อมต่อกับ FastAPI ได้โดยไม่ต้องเขียน HTML/JS เอง

---

### 1.7 ทฤษฎีการจัดการสภาพแวดล้อมด้วย uv

&emsp;&emsp;&emsp;&emsp;`uv` ทำหน้าที่จัดการสภาพแวดล้อมเสมือนและติดตั้งไลบรารีด้วยความเร็วสูง เครื่องมือนี้เขียนด้วย Rust จึงติดตั้งเร็วกว่า `pip` หลายเท่า

&emsp;&emsp;&emsp;&emsp;Virtual Environment (`venv`) ทำหน้าที่แยกไลบรารีของโครงงานออกจากระบบหลัก โครงงานสร้าง `venv` ด้วย `uv venv --python 3.11` แล้วเปิดใช้งานด้วย `.venv\Scripts\Activate.ps1` (Windows) หรือ `source .venv/bin/activate` (macOS/Linux) วิธีนี้ป้องกันความขัดแย้งของรุ่นไลบรารี

&emsp;&emsp;&emsp;&emsp;`uv` ทำหน้าที่ติดตั้งทุกไลบรารีจาก `requirements.txt` ด้วย `uv pip install -r requirements.txt` รายการหลักได้แก่ `fastapi`, `uvicorn`, `ultralytics`, `transformers`, `torch`, `gradio`, `opencv-python` ระบบยึด `requirements.txt` เป็นแหล่งจริง

---

### 1.8 สรุปความสัมพันธ์ของเทคโนโลยี

&emsp;&emsp;&emsp;&emsp;ระบบนำเทคโนโลยีทั้งห้ามาเรียงเป็นลำดับขั้น YOLO ทำหน้าที่ตีกรอบและอ่านตัวเลข SigLIP2 ทำหน้าที่คัดกรองภาพว่าเป็นมิเตอร์น้ำหรือไม่ OpenCV ทำหน้าที่หมุนและปรับแสงเพื่อให้ YOLO เห็นชัด FastAPI ทำหน้าที่รับส่งภาพผ่านเครือข่าย Gradio ทำหน้าที่ให้ผู้ใช้ทดสอบผ่านเบราว์เซอร์ และ uv ทำหน้าที่เตรียมสภาพแวดล้อมให้ทุกชิ้นทำงานร่วมกันได้

---

# บทที่ 2: การพัฒนาระบบอ่านค่ามาตรวัดน้ำเชิงปฏิบัติ

---

### 2.1 ภาพรวมกระบวนการทำงานเชิงปฏิบัติ

&emsp;&emsp;&emsp;&emsp;ระบบเชิงปฏิบัติเริ่มจากผู้ใช้งานถ่ายภาพมาตรวัดน้ำด้วยอุปกรณ์เคลื่อนที่ ภาพถ่ายทำหน้าที่เป็นข้อมูลนำเข้า ขั้นตอนต่อไป SigLIP2 คัดกรองภาพ OpenCV ทดลองหมุน 4 ทิศและปรับแสง 3 แบบ YOLO ตรวจจับตัวเลข กลไกกรองตัดผลซ้อนและแถวแนวตั้ง กลไกให้คะแนนเลือกทิศที่ดีที่สุด กลไกแปลงพิกัดกลับสู่ภาพต้นฉบับ และกลไกความปลอดภัยตรวจสอบภาพกลับหัวก่อนบอกค่าตัวเลข ภาพที่ 2.1 แสดงการไหลดังกล่าว
...
<p align="center"><strong>ภาพที่ 2.1 กระบวนการเชิงปฏิบัติ 12 รูปแบบและการเลือกผู้ชนะ</strong></p>

---

### 2.2 การทดสอบไลบรารีเบื้องต้นก่อนลงมือจริง
...
#### 2.2.1 ทดสอบ SigLIP2 สำหรับ Image Classification
...
#### 2.2.2 ทดสอบ YOLO สำหรับ Object Detection
...
#### 2.2.3 ทดสอบ OpenCV สำหรับ Image Processing
...
#### 2.2.4 ทดสอบ FastAPI สำหรับสร้าง API
...
#### 2.2.5 ทดสอบ Gradio สำหรับสร้าง Web UI
...
### 2.3 ขั้นตอนที่ 1: การรับภาพถ่ายนำเข้า
...
### 2.4 ขั้นตอนที่ 2: การเตรียมชุดข้อมูลและการฝึกแบบจำลอง YOLO
...
#### 2.4.1 การเตรียมชุดข้อมูลจาก Roboflow
...
### 2.5 ขั้นตอนที่ 3: การจัดเตรียมสภาพแวดล้อมและโครงสร้างไฟล์ด้วย uv
...
### 2.6 ขั้นตอนที่ 4: การคัดกรองภาพด้วย SigLIP2
...
### 2.7 ขั้นตอนที่ 5: การปรับปรุงคุณภาพภาพและการจัดการทิศทาง
...
### 2.8 ขั้นตอนที่ 6: การแปลงพิกัดและการกรองข้อมูล
...
#### 2.8.1 การหมุนภาพและการปรับปรุงคุณภาพภาพ
...
#### 2.8.2 การแปลงพิกัดจุดเดี่ยวและกล่องขอบเขต
...
#### 2.8.3 การกรองแถวแนวตั้งและการตรวจสอบสีแดง
...
### 2.9 ขั้นตอนที่ 7: การค้นหาทิศทางและฟิลเตอร์ที่เหมาะสม
...
##### 2.9.1 หมุน 4 ทิศ (0°/90°/180°/270°)

&emsp;&emsp;&emsp;&emsp;ระบบหมุน 4 ทิศเพื่อแก้ภาพเอียงหรือกลับหัว

##### 2.9.2 ปรับแสง 3 แบบ (Orig/CLAHE/HistEq)
...
##### 2.9.4 IoU dedup (threshold 0.45)
...
##### 2.9.6 red_ratio + Scoring

&emsp;&emsp;&emsp;&emsp;ระบบให้คะแนนตามตำแหน่งสีแดง

##### 2.9.7 Scoring (mean_conf × n)
...
### 2.10 ขั้นตอนที่ 8: กลไกความปลอดภัยและการบอกค่าตัวเลข
...
#### 2.10.1 การตรวจจับภาพกลับหัวด้วย flip_guard
...
#### 2.10.2 ฟังก์ชันประมวลผลหลัก read_meter
...
### 2.11 การให้บริการระบบผ่าน FastAPI
...
### 2.12 การพัฒนาส่วนติดต่อผู้ใช้ด้วย Gradio
...
#### 2.13.2 แนวทางการวิเคราะห์และแก้ไขปัญหา
