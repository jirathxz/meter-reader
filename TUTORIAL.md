...
| **In-Process** | กลไกการทำงานภายในระบบ | ทำอะไร? ใช้วิธีไหน? เหตุใดจึงเลือกวิธีนี้? |
...
*PIL Image คืออินพุตมาตรฐานของ SigLIP2* — processor จะ resize ภาพให้เป็น 224×224 pixels และ normalize pixel values โดยอัตโนมัติ การแปลงจาก numpy array ให้เสร็จก่อนส่งเข้า processor จึงเป็นขั้นตอนที่ขาดไม่ได้
...
*`sigmoid` vs `softmax` — ความแตกต่างที่สำคัญ:* SigLIP2 ใช้ Sigmoid loss ในการฝึก (ต่างจาก CLIP ดั้งเดิมที่ใช้ Softmax) ทำให้แต่ละคลาสมีค่าความเชื่อมั่นเป็นอิสระจากกัน — ภาพหนึ่งใบสามารถมีความเชื่อมั่นสูงได้หลายคลาสพร้อมกัน ซึ่งเหมาะกับกรณีที่ภาพมีวัตถุหลายชนิด
...
*เกณฑ์ 0.50 — จุดสมดุลที่ผ่านการทดสอบจริง:* ค่านี้คือจุดสมดุลระหว่าง Precision และ Recall — ต่ำเกินไป (0.30) จะปล่อยภาพผิดประเภทผ่าน สูงเกินไป (0.80) จะกรองภาพถูกต้องที่ถ่ายในแสงน้อยออก

*ผลลัพธ์จาก SigLIP2 ส่งต่ออย่างไร:* ค่า `verified` จะถูกตรวจสอบในฟังก์ชัน `read_meter()` — หากเป็น `False` จะ return error ทันทีโดยไม่เรียก OpenCV หรือ YOLO เลย

---

#### ระบบที่ 2: OpenCV — ช่างภาพดิจิทัล (Image Processing Engine)

> **เหตุผลที่ระบบต้องการ OpenCV**
>
...
*BGR ไม่ใช่ RGB — มรดกทางประวัติศาสตร์ของ OpenCV:* OpenCV สืบทอดระบบสีจากกล้อง CCD รุ่นเก่าที่เก็บข้อมูลในลำดับ BGR และคงรูปแบบนี้ไว้เพื่อความเข้ากันได้ย้อนหลังกับโค้ดเดิมทั่วโลก
...
*CLAHE vs Global HistEq — เลือกใช้กรณีไหน?*
...
ดังนั้นระบบจึงรัน **ทั้งสอง** โดยใช้ OpenCV สร้างภาพตัวแทนในแต่ละสภาวะ แล้วส่งให้โมเดล YOLO ตรวจจับและประเมินว่าสภาวะใดตรวจพบตัวเลขได้ครบถ้วนและมีความเชื่อมั่นสูงสุด

**ขั้นที่ 3 — Output (ผลลัพธ์และการคัดเลือกสมมติฐานภาพ):**

```python
# OpenCV ทำหน้าที่สร้างภาพตัวแทนจาก 12 สมมติฐาน (4 มุมหมุน × 3 รูปแบบการปรับแสง)
# และเรียกใช้โมเดล YOLO ใน eval_orientation() เพื่อเป็นผู้ตัดสินและให้คะแนน
...
# คัดเลือกมุมและภาพตัวแทนที่ดีที่สุดจากคะแนนการตรวจจับของ YOLO
best_angle, best_cand = max(candidates.items(), key=lambda item: item[1]["score"])

# Output: (best_angle, best_prep, best_dets)
# → ส่งต่อให้ขบวนการ Output (YOLO) สรุปผลลัพธ์การอ่านค่าตัวเลข
```

*ความสัมพันธ์ระหว่าง OpenCV และ YOLO ในขั้นตอนนี้:* OpenCV ทำหน้าที่ทางเรขาคณิตและการแปลงภาพ (Image Transformation) เพื่อสร้างภาพตัวแทน 12 รูปแบบ แต่**ผู้ทำหน้าที่ประเมินและตัดสินใจ (Evaluator) คือโมเดลตรวจจับตัวเลข YOLO** ผ่านฟังก์ชัน `eval_orientation()` โดยพิจารณาจากจำนวนตัวเลขที่ตรวจพบและค่าความเชื่อมั่นเฉลี่ย

---

#### ระบบที่ 3: FastAPI — ผู้จัดการระบบ (API Gateway)

> **เหตุผลที่ระบบต้องการ FastAPI**
>
...
>
> *เหตุที่เลือก FastAPI แทน Flask หรือ Django:* FastAPI รองรับ `async/await` และสร้าง API documentation อัตโนมัติ แต่ที่สำคัญกว่าคือมี `run_in_threadpool` ที่ช่วยให้ฟังก์ชัน Python แบบ synchronous (เช่น inference ของ PyTorch) ทำงานในระบบ async ได้โดยไม่บล็อก event loop
...
*หลักการของ `await file.read()`:* FastAPI ทำงานบน async event loop — การใช้ `await` ทำให้ event loop รับ request อื่นในขณะรออ่านไฟล์ได้ ไม่บล็อกระบบ
...
async def read_meter_api(file: UploadFile = File(...)):
    ...
    # ขั้นที่ 2: เรียก read_meter() แบบ non-blocking offload
    result = await run_in_threadpool(read_meter, rgb_img)
    #                 ↑                    ↑         ↑
    #         รอผลลัพธ์โดยไม่บล็อก loop รันใน thread  ฟังก์ชันหลัก
```

*บทบาทของ `run_in_threadpool` — การแยกภาระงานคำนวณหนักออกจาก Event Loop:*
...
sequenceDiagram
    ...
    Note over EV,ML: สถาปัตยกรรมแบบ Non-blocking I/O Event Loop
    ...
    TP->>ML: เริ่มประมวลผลภาพที่ 2 (ขึ้นกับคิวและทรัพยากรระบบ)
...
<p align="center"><strong>แผนภาพที่ 3.1.4: การแยกภาระงานคำนวณหนักออกจาก Event Loop ด้วย ThreadPool ใน FastAPI</strong></p>

&emsp;&emsp;&emsp;&emsp;*หมายเหตุทางเทคนิคเรื่องขีดความสามารถในการทำงานร่วมกัน:* การใช้คำสั่ง `run_in_threadpool()` เป็นการปลดล็อกไม่ให้ฟังก์ชันคำนวณ synchronous (เช่น การคำนวณของ OpenCV หรือการอนุมานของ PyTorch) ไปบล็อกการทำงานของ Event Loop ทำให้เว็บเซิร์ฟเวอร์ยังคงเปิดรับคำขอ I/O ใหม่เข้ามาได้ตลอดเวลา อย่างไรก็ตาม ขีดความสามารถในการรันงานพร้อมกันในระดับฮาร์ดแวร์จริงยังคงถูกจำกัดด้วยขนาดของ Thread Pool, ตัวล็อกส่วนกลางของภาษาไพทอน (Global Interpreter Lock: GIL) ในส่วนการประมวลผลซีพียู และขีดจำกัดของหน่วยความจำ VRAM บนการ์ดจอ

**ขั้นที่ 3 — Output (FastAPI ส่งผลลัพธ์กลับอย่างไร):**

```python
    # ขั้นที่ 3: ส่งคืนผลลัพธ์ JSON ตามสัญญาข้อมูลที่เป็นมาตรฐาน (API Contract)
    # ฟังก์ชัน read_meter() ได้กำหนดฟิลด์ "success": True/False ที่ระดับบนสุดไว้อย่างสม่ำเสมอ
    return result
```

*ปลายทางของ JSON Response จาก FastAPI:* ผลลัพธ์ถูกส่งกลับไปยัง client ที่เรียก — ซึ่งอาจเป็น Gradio UI (ผ่าน HTTP POST), แอปพลิเคชันมือถือของเจ้าหน้าที่, หรือระบบสารสนเทศหลังบ้าน (Backend Database)  
*ข้อพิจารณาด้านเครือข่ายและการใช้งานจริง:* ในระหว่างการพัฒนาและทดสอบ ระบบจะทำงานบนเครื่องเฉพาะที่ (`http://127.0.0.1:8000`) สำหรับการนำไปใช้งานภาคสนามจริง เซิร์ฟเวอร์ FastAPI จะต้องถูกติดตั้งบนเครื่องแม่ข่ายหรือระบบคลาวด์ พร้อมโดเมนสาธารณะหรือระบบ VPN และเปิดใช้งานโพรโทคอล HTTPS เพื่อความปลอดภัยในการรับส่งข้อมูลจากสมาร์ทโฟน

---

#### ระบบที่ 4: Gradio — หน้าต่างสู่โลก (Web UI)

> **เหตุผลที่ระบบต้องการ Gradio**
>
...
>
> *เหตุที่เลือก Gradio แทน Streamlit หรือ React:* Gradio ออกแบบมาเฉพาะสำหรับ ML Demo — มี component `gr.Image`, `gr.JSON`, `gr.Textbox` พร้อมใช้, รองรับ async โดยธรรมชาติ, และแยก UI ออกจาก backend FastAPI ได้สะอาด ทำให้ทั้งสองระบบ **ทำงานได้อิสระจากกัน** — นักพัฒนาทดสอบ API ผ่าน curl ได้โดยไม่ต้องเปิด Gradio
...
*การแปลงเป็น JPEG ก่อนส่ง — เหตุผลด้านโปรโตคอล:* FastAPI รับข้อมูลในรูปแบบ multipart/form-data (ไฟล์ภาพ) — Gradio ต้องแพ็กข้อมูลเป็นไฟล์ก่อนส่งผ่าน HTTP POST เหมือนกับที่แอปมือถือทำ
...
*ปลายทางสุดท้ายของ Gradio:* **ผู้ใช้งานจริง** — เจ้าหน้าที่ภาคสนามที่เปิดเบราว์เซอร์ไปที่ `http://127.0.0.1:7860` อัปโหลดภาพ และอ่านผลลัพธ์ได้ทันที
...
&emsp;&emsp;&emsp;&emsp;*เหตุผลความจำเป็น:* ระบบถูกออกแบบด้วยสถาปัตยกรรมแบบแยกส่วนระหว่างส่วนติดต่อผู้ใช้และบริการประมวลผล การรับข้อมูลภาพผ่าน HTTP Multipart Form Data ช่วยให้สามารถส่งข้อมูลไบนารีของไฟล์ได้โดยไม่จำเป็นต้องแปลงเป็น Base64 ซึ่งหลีกเลี่ยง overhead จากการเข้ารหัส Base64 ที่โดยทั่วไปทำให้ขนาดข้อมูลเพิ่มขึ้นประมาณหนึ่งในสาม พร้อมทั้งลดภาระการจัดสรรหน่วยความจำของเซิร์ฟเวอร์ในการถอดรหัสสตริงขนาดใหญ่
...
&emsp;&emsp;&emsp;&emsp;**ความสำคัญของการทดสอบด้วยการแสดงภาพผลลัพธ์ (Visual Inspection Rationale):** ในขบวนการปรับปรุงภาพ การหมุนภาพ 4 ระนาบ (0°, 90°, 180°, 270°) และการปรับแต่งคอนทราสต์ 3 รูปแบบ (Orig, CLAHE บน LAB, และ HistEq บน YCrCb) ส่งผลกระทบโดยตรงต่อโครงสร้างทางเรขาคณิตและระดับความเข้มของพิกเซล หากทดสอบเฉพาะมิติอาเรย์ของภาพผ่านฟังก์ชัน เช่น `assert rot_90.shape[:2] == (w, h)` ผู้พัฒนาจะไม่สามารถทราบได้เลยว่าฟิลเตอร์ CLAHE ก่อให้เกิดสัญญาณรบกวนเกินขนาดในบริเวณตัวเลขหรือไม่ หรือฟิลเตอร์ HistEq ทำให้บริเวณที่มีแสงจ้ากลายเป็นสีขาวโพลนจนลายเส้นตัวเลขขาดหายหรือไม่ นอกจากนี้ การตรวจสอบฟังก์ชันวิเคราะห์ทิศทางข้อความบนหน้าปัด (`detect_dial_text_orientation`) จำเป็นต้องมองเห็นกรอบคอนทัวร์ที่ตรวจจับได้จริงว่าสอดคล้องกับสัญลักษณ์ $m^3$ หรือไม่

&emsp;&emsp;&emsp;&emsp;ทั้งนี้ เพื่อให้ผู้พัฒนาสามารถตรวจสอบเชิงประจักษ์ได้อย่างเป็นรูปธรรม การทดสอบ Checkpoint 2 จึงได้รับการออกแบบให้แสดงผลลัพธ์ในรูปแบบ **แผงตรวจผลลัพธ์ด้วยสายตา 8 ช่อง (8 Visual Inspection Panels)** จัดวางเป็นตาราง 2 แถว 4 คอลัมน์ (แถวที่ 1: การหมุนภาพ 4 ทิศทาง, แถวที่ 2: ฟิลเตอร์ปรับแสง 3 รูปแบบ ควบคู่กับภาพครอปตรวจสอบข้อความ $m^3$) ซึ่งแผงตรวจ 8 ช่องนี้มีวัตถุประสงค์เพื่อการตรวจสอบคุณภาพของแต่ละฟังก์ชันทางสายตาโดยเฉพาะ แตกต่างจาก **12 รูปแบบสมมติฐานการประมวลผล (12 Candidate Configurations: 4 มุมหมุน × 3 ฟิลเตอร์)** ซึ่งเป็นชุดข้อมูลที่ถูกประมวลผลจริงในไปป์ไลน์และส่งต่อให้โมเดล YOLO ทำการประเมินคะแนนต่อไป
...
&emsp;&emsp;&emsp;&emsp;ผลลัพธ์จากการรันสคริปต์ทดสอบจะถูกบันทึกเป็นไฟล์ภาพแผงตรวจด้วยสายตา 8 ช่อง (8 Visual Inspection Panels) ซึ่งแสดงให้เห็นประสิทธิภาพของการหมุนภาพ 4 ทิศทาง การปรับฟิลเตอร์แสง 3 รูปแบบ และการตรวจจับข้อความหน้าปัด เพื่อให้ผู้พัฒนาตรวจสอบคุณภาพได้ชัดเจน ดังแสดงในภาพที่ 3.3:

<p align="center">
  <img src="media/checkpoint2_process_test.png" alt="ผลการทดสอบเชิงประจักษ์ใน Checkpoint 2 แผงตรวจผลลัพธ์ด้วยสายตา 8 ช่อง แสดงการหมุนภาพ 4 ระนาบ การปรับคอนทราสต์ 3 รูปแบบ และการตรวจจับข้อความหน้าปัด" width="800"/>
  <br>
  <strong>ภาพที่ 3.3: ภาพผลลัพธ์การทดสอบเชิงประจักษ์ของขบวนการปรับปรุงภาพ (Checkpoint 2: Transformation & Dial Text Visual Test) ผ่านแผงตรวจผลลัพธ์ด้วยสายตา 8 ช่อง แสดงการหมุน 4 ระนาบ การปรับคอนทราสต์ 3 รูปแบบ และการตรวจจับข้อความหน้าปัด m³</strong>
  <br>
  <em>ที่มา: จากการทดสอบระบบจริงในโครงงานนี้</em>
</p>
...
&emsp;&emsp;&emsp;&emsp;1. **การตรวจจับและจำแนกตัวเลขดิบด้วยแบบจำลอง YOLO26 (YOLO26 Digit Detection):**  
&emsp;&emsp;&emsp;&emsp;*เหตุผลความจำเป็น:* สถาปัตยกรรม YOLO26 เป็น Single-stage Object Detector แบบ Anchor-free ที่มีความเร็วสูงและแม่นยำต่อวัตถุขนาดเล็ก ทำหน้าที่ตีกรอบสี่เหลี่ยม (Bounding Box Localization) และจำแนกตัวเลข 0–9 (Digit Classification) เท่านั้น โดย**แบบจำลองไม่ได้จำแนกว่าหลักใดเป็นจำนวนเต็มหรือทศนิยมในขั้นตอนนี้** การมองตัวเลขแต่ละหลักเป็นวัตถุอิสระทำให้ระบบสามารถระบุพิกัดและจำแนกตัวเลขได้โดยตรงจากภาพถ่ายเต็มเฟรมโดยไม่ต้องตัดขอบหน้าปัดล่วงหน้า และทนทานต่อการขีดข่วนหรือคราบเปื้อนบนตัวเลขได้ดีกว่า OCR แบบดั้งเดิม
...
&emsp;&emsp;&emsp;&emsp;*เหตุผลความจำเป็น:* กล่องตัวเลขที่ตรวจจับได้จาก YOLO จะไม่มีลำดับตามหลักตัวเลข แต่การอ่านค่ามาตรวัดน้ำจำเป็นต้องเรียงตามหลักเลขนัยสำคัญจากหลักมากไปหาหลักน้อย (ซ้ายไปขวา) ระบบจึงบังคับเรียงลำดับรายการกล่องตามพิกัดแนวนอน `xmin` (`bbox[0]`) จากค่าน้อยไปมากเสมอ เพื่อให้ค่าสตริงตัวเลขที่ได้ตรงกับค่าที่มนุษย์อ่านจริง

&emsp;&emsp;&emsp;&emsp;5. **การวิเคราะห์หลักทศนิยมสีแดงและการคำนวณค่าทศนิยม (HSV Red Ratio Decimal Calculation):**  
&emsp;&emsp;&emsp;&emsp;*เหตุผลความจำเป็น:* มาตรวัดน้ำมาตรฐานส่วนใหญ่จะมีแถบตัวเลขหลักหน่วยเป็นสีดำ และหลักทศนิยม (ลิตร) เป็นตัวเลขสีแดงอยู่ทางขวาสุด 1–3 หลัก ฟังก์ชัน `red_ratio` จะนำกล่องตัวเลขที่ผ่านการเรียงลำดับซ้ายไปขวาแล้ว มาแปลงพื้นที่พิกเซลเป็นปริภูมิสี HSV และวัดสัดส่วนพิกเซลสีแดง หากตรวจพบว่าหลักขวาสุดมีสัดส่วนสีแดงสูงเกินเกณฑ์ ($\ge 0.25$) ระบบจึงกำหนดให้หลักนั้นเป็นหลักทศนิยม (Decimal Digit) พร้อมคำนวณค่าตัวเลขอ่านจริง (เช่น $\text{Reading} / 10^{N_{\text{dec}}}$) และให้คะแนนโบนัส +5% แก่สมมติฐานนั้น แต่หากพบว่าหลักสีแดงไปอยู่ซ้ายสุด แสดงว่าภาพอาจกำลังกลับหัว 180° ระบบจะตัดคะแนนลง -50% เพื่อป้องกันการตัดสินใจผิดพลาด
...
def run_checkpoint3_visual():
    ...
    # ยืนยันพิกัดแนวนอน xmin (bbox[0]) เรียงลำดับจากซ้ายไปขวา
    x_positions = [d["bbox"][0] for d in clean_dets]
    assert x_positions == sorted(x_positions), "ความล้มเหลว: กล่องตัวเลขต้องเรียงพิกัดจากซ้ายไปขวา"
...
```
<p align="center"><strong>ซอร์สโค้ดที่ 3.8: การบูรณาการ 3 ขบวนการเข้าเป็นฟังก์ชัน <code>read_meter</code> พร้อมระบบกลไกความปลอดภัยและสัญญาข้อมูล success</strong></p>
...
# หมายเหตุ: การอนุญาต allow_origins=["*"] เป็นการกำหนดค่าสำหรับการพัฒนาและทดสอบในเครื่องเฉพาะที่ (Local Prototyping)
# ในสภาวะแวดล้อมการใช้งานจริง (Production) ควรจำกัดเฉพาะโดเมนของแอปพลิเคชันที่ได้รับอนุญาตอย่างเคร่งครัด
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
&emsp;&emsp;&emsp;&emsp;*หมายเหตุการตั้งค่า CORS และความมั่นคงปลอดภัย:* การกำหนด `allow_origins=["*"]` ใน `CORSMiddleware` เป็นการกำหนดค่าสำหรับการพัฒนาและทดสอบในสภาพแวดล้อมเฉพาะที่ (Development Environment) เพื่อความสะดวกในการเชื่อมต่อกับส่วนต่อประสานผู้ใช้ Gradio หรือเครื่องมือทดสอบ เช่น Postman และเว็บเบราว์เซอร์ สำหรับการนำระบบไปใช้งานจริงในสภาวะแวดล้อมจริง (Production Environment) จะต้องจำกัดโดเมนต้นทาง (`allow_origins`) ให้ระบุเฉพาะชื่อโฮสต์ของเว็บแอปพลิเคชันหรือที่อยู่ IP ของเครือข่ายองค์กรที่ได้รับอนุญาตเท่านั้นเพื่อป้องกันช่องโหว่ด้าน Cross-Origin Request Forgery
...
def predict(image: Image.Image | None):
    ...
    if not success and not reading:
        warn_text = "\n".join(f"- ❌ {w}" for w in warnings) if warnings else "❌ ไม่สามารถประมวลผลอ่านค่าได้"
        return "ไม่สามารถอ่านค่าได้", "0.00%", warn_text
...
#### 3.6.1 การทดสอบการทำงานของระบบแบบบูรณาการ (Integration & End-to-End Verification)

&emsp;&emsp;&emsp;&emsp;เพื่อยืนยันว่าการเชื่อมต่อระหว่าง 3 ขบวนการหลัก (Preprocessing ด้วย SigLIP2, Process ด้วย OpenCV และ Output ด้วย YOLO26) สามารถทำงานประสานกันได้อย่างถูกต้องตามสถาปัตยกรรมที่ออกแบบไว้โดยไม่เกิดข้อผิดพลาดด้านมิติข้อมูลหรือการส่งผ่านข้อมูลที่ผิดพลาด ระบบจึงได้รับการทดสอบแบบครบวงจร (End-to-End Pipeline) ด้วยสคริปต์ทดสอบอัตโนมัติผ่านเครื่องมือ `uv` ดังแสดงในชุดคำสั่งที่ 3.1:

```powershell
# รันการทดสอบและยืนยันการทำงานของระบบอ่านค่าอัตโนมัติแบบครบวงจรบนชุดภาพสาธิต
uv run python validate.py
```
<p align="center"><strong>ชุดคำสั่งที่ 3.1: การสั่งทดสอบการทำงานของระบบแบบบูรณาการครบวงจรด้วยเครื่องมือ <code>uv</code></strong></p>

#### 3.6.2 การตรวจจับข้อผิดพลาดและการจัดการกรณีข้อยกเว้น (Error Handling & Debugging Matrix)

&emsp;&emsp;&emsp;&emsp;ในการทำงานจริงกับภาพถ่ายภาคสนาม ระบบอาจเผชิญกับสภาวะแวดล้อมที่ผิดปกติหรือไม่เป็นไปตามอุดมคติ เช่น ภาพเบลอ วัตถุแปลกปลอม แสงสะท้อนจ้า หรือตัวเลขแนวตั้ง ตารางที่ 3.1 สรุปแนวทางการวิเคราะห์อาการผิดปกติ สาเหตุ จุดตรวจสอบ และวิธีแก้ไขปัญหาสำหรับการดูแลรักษาระบบ:
...
#### 3.6.3 คู่มือการติดตั้งและสั่งทำงานระบบ (System Execution Guide)

&emsp;&emsp;&emsp;&emsp;3.6.3.1 การเปิดใช้งานเซิร์ฟเวอร์บริการ FastAPI Backend (Terminal 1): เริ่มต้นการทำงานของเซิร์ฟเวอร์ที่พอร์ต 8000 ดังแสดงในชุดคำสั่งที่ 3.2:

```powershell
uv run python main.py
```
<p align="center"><strong>ชุดคำสั่งที่ 3.2: การสั่งเริ่มต้นทำงานเซิร์ฟเวอร์ FastAPI Backend ด้วยเครื่องมือ <code>uv</code></strong></p>
...
&emsp;&emsp;&emsp;&emsp;3.6.3.2 การเปิดใช้งานส่วนติดต่อผู้ใช้ Gradio Web UI (Terminal 2): เริ่มต้นการทำงานของหน้าเว็บที่พอร์ต 7860 ดังแสดงในชุดคำสั่งที่ 3.3:

```powershell
uv run python gradio_app.py
```
<p align="center"><strong>ชุดคำสั่งที่ 3.3: การสั่งเริ่มต้นทำงานส่วนติดต่อผู้ใช้ Gradio Web UI ด้วยเครื่องมือ <code>uv</code></strong></p>
...
&emsp;&emsp;&emsp;&emsp;*ข้อสังเกตด้านเครือข่ายระหว่างการพัฒนาและการใช้งานจริง:* การเข้าถึงผ่าน URL `http://127.0.0.1:7860` และ `http://127.0.0.1:8000` เป็นการทำงานบนเครื่องเฉพาะที่ (Localhost) สำหรับการพัฒนาและสาธิตเชิงวิศวกรรม หากต้องการนำไปให้บริการจริงแก่เจ้าหน้าที่จดค่าน้ำภาคสนามผ่านสมาร์ทโฟน ระบบจะต้องติดตั้งบนเครื่องแม่ข่ายหรือระบบคลาวด์ที่มีหมายเลขไอพีสาธารณะ (Public IP) หรือเชื่อมต่อผ่านเครือข่ายเสมือนส่วนตัว (VPN) พร้อมเปิดใช้งานโพรโทคอล HTTPS เพื่อความปลอดภัยสูงสุด

#### 3.6.4 บทสรุปการทดสอบระบบและการเชื่อมโยงสู่การประเมินผลเชิงลึกในบทที่ 4 (Summary & Forward Linkage to Chapter 4)

&emsp;&emsp;&emsp;&emsp;ตารางที่ 3.2 สรุปผลสัมฤทธิ์เบื้องต้นของการทำงานของระบบอ่านค่ามาตรวัดน้ำอัตโนมัติ (Preliminary Verification Summary) เพื่อยืนยันว่าระบบและโมดูลต่างๆ สามารถทำงานร่วมกันได้จริงตามข้อกำหนดเชิงวิศวกรรม:
...
&emsp;&emsp;&emsp;&emsp;*หมายเหตุสำคัญและการเชื่อมโยงสู่บทที่ 4:* ผลการทดสอบในตารางที่ 3.2 มีวัตถุประสงค์เพื่อยืนยันความพร้อมในการทำงานของฟังก์ชันระบบ (Functional Verification) สำหรับ**การประเมินประสิทธิภาพเชิงลึกอย่างเป็นทางการ การทดสอบบนชุดข้อมูลทดสอบที่แยกต่างหากอย่างสมบูรณ์ (Held-out Test Split, n=194), การวิเคราะห์เชิงตัดทอนเปรียบเทียบแต่ละโมดูล (Ablation Study M0 ถึง M7), ตัวชี้วัด mAP@50-95, และการวิเคราะห์ข้อจำกัดและข้อผิดพลาดเชิงลึก จะถูกนำเสนอและอภิปรายอย่างละเอียดในบทที่ 4 (ผลการศึกษา อภิปรายผล และข้อเสนอแนะ)** ต่อไป
