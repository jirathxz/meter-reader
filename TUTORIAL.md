...
| 5 | **เวลาประมวลผล (System Latency)** | **5,828.8 ms ต่อภาพ** (12 สมมติฐาน)<br>*(Primary Official Benchmark)* | **533.2 ms ต่อภาพ** (Single-pass)<br>*(Official Single-pass Benchmark)* | วัดบน CPU AMD Ryzen 5 8645HS ที่ 960×960 (โดย Ablation M0 Baseline เท่ากับ 421.1 ms) |
...
| **เวลาประมวลผลมาตรฐาน (System Latency)** | **533.2 ms ต่อภาพ**<br>*(Official Single-pass Benchmark)* | **5,828.8 ms ต่อภาพ** (12 สมมติฐาน)<br>*(Primary Official Benchmark)* | สภาวะคงที่ (Steady-state) บน CPU AMD Ryzen 5 ที่ 960×960 (Ablation M0 Baseline = 421.1 ms) |
...
| `rotate-4` (4มุม+orig) | **100.0% (7/7)** *(บนชุดสาธิต)* | 0.846 | 2.95s | การหมุน 4 ทิศทางช่วยกู้คืนภาพเอียง ส่งผลให้อัตราความสำเร็จเพิ่มขึ้นจาก 57.1% เป็น 100.0% ในชุดสาธิตนี้ |
...
| **Official Single-pass Benchmark** | **533.2 ms ต่อภาพ** | การอนุมานเพียงสมมติฐานเดียว (0° ต้นฉบับ) บนชุดทดสอบที่กันออกจากการฝึก (Held-out Test Split, n=120) | **เกณฑ์เปรียบเทียบมาตรฐานรอบเดียว:** ใช้ประเมินความเร็วพื้นฐานของท่อประมวลผลเมื่อเทียบกับระบบทั่วไปที่ไม่หมุนค้นหา |
...
> [!NOTE]
> **ข้อกำหนดการอ้างอิงเกณฑ์มาตรฐานเวลา:** โครงงานกำหนดให้เฉพาะเวลาประมวลผลของระบบบูรณาการเต็มรูปแบบในสภาวะคงที่ (**5,828.8 ms หรือประมาณ 5.83 วินาทีต่อภาพ**) เท่านั้นที่ถือเป็น **Primary Official Benchmark** ของระบบที่นำเสนอ โดยค่า 533.2 ms ถือเป็น *Official Single-pass Benchmark* และค่า 421.1 ms ถือเป็น *Ablation M0 Baseline* ซึ่งอยู่ภายใต้โปรโตคอลการวัดคนละสภาวะกัน
...
| **RQ5: Latency-Accuracy Trade-off**<br>ความแม่นยำที่เพิ่มขึ้นแลกกับเวลาประมวลผลเท่าใด? | การวัดเวลาประมวลผลเชิงลึก (Latency Profiling) บน CPU AMD Ryzen 5 | Ablation M0 Baseline ใช้เวลา 421.1 ms (Exact Match 42.9%) ขยายเป็น 5,828.8 ms ใน Official Full Pipeline M7 (100.0% บนชุดภาพสาธิต 7 ภาพ) | ผลดังกล่าวแสดงถึง trade-off ระหว่างความแม่นยำและเวลาในการประมวลผล โดยการประยุกต์ใช้ multi-angle inference ทำให้เวลาเพิ่มขึ้นเมื่อเทียบกับ baseline |
...
| การค้นหา 12 สมมติฐานสัมพันธ์กับอัตราความแม่นยำที่สูงกว่า Single-pass บนชุดการทดลอง | Empirical | ผลการทดลอง Progressive Ablation: M0 (42.9%) vs M2 (71.4%) vs M4 (100.0% บนชุดภาพสาธิต 7 ภาพ) (ดูตารางที่ 4.5 ในหัวข้อ 4.1.3) | Supported |
...
| เวลาประมวลผลของระบบบน CPU (Primary Benchmark: 5.83 s/ภาพ) | Empirical | Primary Benchmark: Official Full Pipeline M7 = 5,828.8 ms (~5.83 s ในสภาวะคงที่) เทียบกับ Official Single-pass = 533.2 ms (บน Test Split n=120) และ Ablation M0 Baseline = 421.1 ms (บนชุดสาธิต n=7) (ดูหัวข้อ 4.1.4 และตารางที่ 4.5) | Supported |
