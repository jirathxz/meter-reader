...
&emsp;&emsp;&emsp;&emsp;1.3.2.1 **คำถามการวิจัยที่ 1 (RQ1 - Object Detection Performance):** แบบจำลอง YOLO26m ที่ผ่านการฝึกฝนด้วยภาพหน้าปัดมาตรวัดน้ำ มีประสิทธิภาพในการตรวจจับตัวเลขโดด (0–9) ในระดับใด เมื่อวัดด้วย Precision, Recall, F1-score, mAP@50 และ mAP@50:95 บนชุดข้อมูลทดสอบอิสระ?
&emsp;&emsp;&emsp;&emsp;1.3.2.2 **คำถามการวิจัยที่ 2 (RQ2 - Multi-hypothesis Search Effectiveness):** การใช้กลยุทธ์ค้นหาเชิงสมมติฐานพหุคูณ 12 รูปแบบ (4 ทิศทางการหมุน × 3 ฟิลเตอร์ปรับปรุงภาพ) ช่วยเพิ่มอัตราความสำเร็จในการอ่านค่า (Success Rate) และความแม่นยำระดับทั้งตัวเลข (Exact Reading Accuracy) เมื่อเทียบกับการประมวลผลภาพเดี่ยวมุมเดิม (Baseline Single-pass) มากน้อยเพียงใด?
...
&emsp;&emsp;&emsp;&emsp;1.3.2.4 **คำถามการวิจัยที่ 4 (RQ4 - Zero-shot Verification Impact):** การประยุกต์ใช้แบบจำลองภาษา-ภาพ SigLIP2 แบบ Zero-shot มีความสามารถในการคัดกรองภาพที่ไม่ใช่มิเตอร์น้ำ (Out-of-Distribution Rejection) และช่วยป้องกันการทำนายผิดพลาดได้เพียงใด?
...
1.4.2 **ขอบเขตด้านชุดข้อมูลและการแบ่งข้อมูล (Dataset & Splitting Protocol):**
...
1.4.3 **ขอบเขตด้านฮาร์ดแวร์และซอฟต์แวร์:**
&emsp;&emsp;1.4.3.1 **การฝึกฝนแบบจำลองตรวจจับ (Training Phase):** ดำเนินการบนสภาพแวดล้อมคลาวด์ Google Colab โดยใช้หน่วยประมวลผลกราฟิก NVIDIA Tesla T4 (16GB GDDR6 VRAM)
&emsp;&emsp;1.4.3.2 **การประมวลผลใช้งานจริง (Inference Pipeline):** ได้รับการออกแบบให้ทำงานได้ทั้งบนเครื่องคอมพิวเตอร์ทั่วไปที่ใช้ CPU (เช่น สถาปัตยกรรม x86-64 / ARM) และเครื่องที่มี GPU รองรับ NVIDIA CUDA โดยมีการตรวจวัดเวลาประมวลผลแบบแยกส่วนอย่างละเอียด
...
3. ตรวจสอบชนิดไฟล์และไบนารีภาพด้วยไลบรารี PIL
   3.1 กรณีไฟล์ไม่ถูกต้อง: ระบบปฏิเสธคำขอด้วยรหัสข้อผิดพลาด HTTP 415 หรือ 422
...
3. ตรวจสอบว่าภาพดังกล่าวได้รับการจำแนกเป็นมาตรวัดน้ำด้วยค่าความเชื่อมั่นไม่น้อยกว่า 0.50 หรือไม่
   3.1 กรณีไม่ใช่มาตรวัดน้ำ: แจ้งเตือนผู้ใช้และยุติกระบวนการเพื่อประหยัดทรัพยากร GPU
...
2. ตรวจสอบมุมองศาของการหมุน (angle)
   2.1 กรณีหมุน 90 องศา: คำนวณพิกัดใหม่เป็น (y, H - x)
   2.2 กรณีหมุน 180 องศา: คำนวณพิกัดใหม่เป็น (W - x, H - y)
...
2. ตรวจสอบทิศทางการเรียงตัวของแถวตัวเลขด้วยฟังก์ชัน is_vertical
   2.1 กรณีเป็นแถวแนวตั้ง: ตัดทิ้งทันทีเนื่องจากไม่ใช่แถวตัวเลขบนหน้าปัด
...
4. ตรวจสอบสัดส่วนพิกเซลสีแดง (red_ratio)
   4.1 หากหลักสีแดงอยู่ขวาสุด: มอบคะแนนโบนัสเพิ่มร้อยละ 5
...
2. เปรียบเทียบคะแนนกับมุมปกติ 0 องศาตามกฎ Margin Rule
   2.1 กรณีมุมอื่นได้คะแนนสูงกว่ามุม 0 องศาเกินเกณฑ์ 0.12: คัดเลือกมุมที่ได้คะแนนสูงสุดเป็นผู้ชนะ
...
2. เปรียบเทียบความสัมพันธ์ของตัวเลขผ่านตารางคู่สมมาตร FLIP_MAP
   2.1 กรณีค่าตัวเลขสอดคล้องกันตามปกติ: ยืนยันว่าทิศทางภาพถูกต้อง
...
2. พิจารณาตามเกณฑ์ความเชื่อมั่นที่กำหนด (CONF_RELIABLE เท่ากับ 0.60)
   2.1 กรณีค่าความเชื่อมั่นตั้งแต่ 0.60 ขึ้นไป: ยอมรับผลลัพธ์เนื่องจากมีความน่าเชื่อถือสูง
...
2. ตรวจสอบว่าค่าเบี่ยงเบนมาตรฐานอยู่ในเกณฑ์ที่กำหนดหรือไม่ (ไม่เกิน 0.10)
   2.1 กรณีเกินเกณฑ์: แจ้งเตือนว่าแนวแถวตัวเลขเอียงผิดปกติ
   2.2 กรณีอยู่ในเกณฑ์: วิเคราะห์สัดส่วนสีแดงในปริภูมิสี HSV ที่กล่องขวาสุด
3. ตรวจสอบตำแหน่งของหลักทศนิยมสีแดง
   3.1 กรณีหลักสีแดงอยู่ขวาสุด: ยืนยันโครงสร้างหน้าปัดถูกต้อง พร้อมมอบโบนัสร้อยละ 5
...
3. **การแจกแจงกรณีข้อผิดพลาดจริง (Empirical Failure Taxonomy):** จากการวิเคราะห์เชิงลึกใน 39 ภาพที่อ่านไม่ตรงเฉลยครบทุกหลัก (บันทึกใน `reviews/test_set_evaluation.json`) พบสาเหตุหลัก 3 ประการ:
   3.1 **ตัวเลขกึ่งกลางรอบหมุน (Half-turned Wheel Roll):** พบมากที่สุดถึง 64.1% (25 จาก 39 ภาพ) เช่น ภาพ `0b61cbb2...` เฉลยคือ `000108865` แต่ระบบอ่านได้ `000108863` เนื่องจากลูกล้อหลักสุดท้ายกำลังหมุนเปลี่ยนผ่าน ส่งผลให้ตัวเลขมีความกำกวมเชิงภาพ แม้ระบบอ่านถูกต้องถึง 8 ใน 9 หลักก็ตาม
   3.2 **โครงสร้างตัวเลขคล้ายคลึงภายใต้คราบสกปรก (Structural Ambiguity):** พบ 25.6% (10 จาก 39 ภาพ) เช่น การสับสนระหว่างเลข 5 กับ 6 ในภาพ `0bf86f8b...` (อ่านได้ `00000165` จากเฉลย `00000155`) หรือการสับสนระหว่างเลข 8 กับ 9 ในสภาพแสงมืด
...
1. **การค้นหาพหุคูณ 4 ทิศทางการหมุน (เปรียบเทียบ M0 กับ M1: Multi-Angle Search):**
   1.1 *What it does (หน้าที่การทำงาน):* อัลกอริทึมหมุนภาพนำเข้า 4 มุมหลัก ได้แก่ 0°, 90°, 180°, และ 270° ก่อนส่งเข้าตัวตรวจจับ
   1.2 *Why it is used (เหตุผลเชิงเทคนิค):* กล้องสมาร์ทโฟนของเจ้าหน้าที่ภาคสนามมักถ่ายภาพในทิศทางที่ไม่แน่นอน หรือตัวเรือนมาตรวัดน้ำถูกติดตั้งในแนวดิ่งตามแนวท่อ การอาศัยมุม 0° เพียงอย่างเดียว (M0) ส่งผลให้ Exact Reading Accuracy ต่ำเพียง 42.9%
...
2. **การปรับปรุงคอนทราสต์แสงเชิงพื้นที่ (เปรียบเทียบ M1 กับ M2: Contrast Preprocessing):**
   2.1 *What it does (หน้าที่การทำงาน):* ผสานการแปลงภาพแบบ CLAHE (Contrast Limited Adaptive Histogram Equalization) และ Histogram Equalization เข้ากับภาพต้นฉบับ ทำให้เกิดสมมติฐานการประมวลผล 12 รูปแบบ (4 มุม × 3 ฟิลเตอร์)
   2.2 *Why it is used (เหตุผลเชิงเทคนิค):* หน้าปัดมาตรวัดน้ำภาคสนามมักมีคราบตะไคร่น้ำ คราบฝุ่น หรือแสงสะท้อนจ้าบนกระจกครอบ CLAHE ช่วยเพิ่มคอนทราสต์เฉพาะจุดโดยไม่ขยายสัญญาณรบกวน (Noise)
...
3. **การคัดกรองเค้าโครงแนวตั้ง (เปรียบเทียบ M2 กับ M3: Vertical Layout Guard - `is_vertical`):**
   3.1 *What it does (หน้าที่การทำงาน):* ตรวจสอบอัตราส่วนความกว้างต่อความสูงของกล่องรวมตัวเลข (Bounding Box Span Ratio: $\Delta y / \Delta x$) หากตัวเลขเรียงตัวตามแนวดิ่งเกินเกณฑ์จะถูกตัดทิ้งทันที
   3.2 *Why it is used (เหตุผลเชิงเทคนิค):* หน้าปัดแบบลูกล้อกลไกเรียงตัวในแนวนอนเท่านั้น เมื่อภาพหมุนผิดมุม 90° หรือ 270° แบบจำลองอาจตรวจจับตัวเลขบางตัวและระบุเป็นผลลัพธ์ที่ผิดพลาด
...
4. **การคำนวณโบนัสตัวเลขสีแดง (เปรียบเทียบ M3 กับ M4: Red Digit Ratio Bonus - `red_ratio`):**
   4.1 *What it does (หน้าที่การทำงาน):* สกัดแถบสี HSV ในขอบเขตตัวเลขหลักขวาสุด เพื่อตรวจวัดสัดส่วนพิกเซลสีแดง (`red_ratio`) และให้คะแนนถ่วงน้ำหนักพิเศษในสูตรคัดเลือกสมมติฐานที่ดีที่สุด (`score_reading`)
   4.2 *Why it is used (เหตุผลเชิงเทคนิค):* มาตรวัดน้ำประปาส่วนใหญ่มีหลักทศนิยมเป็นสีแดงอยู่ทางขวาสุด หากระบบอ่านภาพกลับหัว 180° ตัวเลขสีแดงจะไปปรากฏอยู่ซ้ายสุด การตรวจจับสัดส่วนสีแดงช่วยสนับสนุนการระบุทิศทางหัว-ท้ายของชุดตัวเลข
...
5. **กลไกความปลอดภัยและความคงสภาพข้อมูล (เปรียบเทียบ M5 กับ M7: Safety & Verification Guards):**
   5.1 *What it does (หน้าที่การทำงาน):* ประกอบด้วย `flip_guard` (ตรวจจับการกลับหัวเชิงสมมาตร), `cross_check_digits` (ตรวจสอบความสอดคล้องของตัวเลขข้ามสมมติฐาน), และ SigLIP2 Zero-shot Classifier (ตัวคัดกรองภาพนำเข้า)
   5.2 *Why it is used (เหตุผลเชิงเทคนิค):* ในระบบการจัดเก็บข้อมูลจริง ความเสี่ยงจากการอ่านตัวเลขผิดพลาด (False Positive) ก่อให้เกิดผลกระทบมากกว่าการปฏิเสธเพื่อให้ถ่ายภาพใหม่ กลไกความปลอดภัยทำหน้าที่ตรวจสอบความสอดคล้องขั้นสุดท้าย
...
1. **โปรโตคอล ก (Initial Run with Cold-start Overhead — เฉลี่ย 10.3 วินาทีต่อภาพ):**
   1.1 *ค่าที่วัดได้จริง:* เวลารวมทั้ง 7 ภาพเท่ากับ 72,339 มิลลิวินาที ส่งผลให้เวลาเฉลี่ยเท่ากับ **10,334 ms (10.33 วินาทีต่อภาพ)** ตามข้อมูลดิบในตารางที่ 3.7
...
2. **โปรโตคอล ข (Repeated Benchmark Run — เฉลี่ย 8.3 วินาทีต่อภาพ):**
   2.1 *ค่าที่วัดได้จริง:* ค่าเฉลี่ย **8.34 วินาทีต่อภาพ** (ช่วงเวลาต่ำสุด 4.9 วินาที, เฉลี่ย 8.3 วินาที, สูงสุด 26.7 วินาที) ตามที่รายงานในตารางที่ 3.2
...
3. **โปรโตคอล ค (Steady-State Warm Run / Controlled Ablation — เฉลี่ย 5.8 วินาทีต่อภาพ):**
   3.1 *ค่าที่วัดได้จริง:* ค่าเฉลี่ย **5,828.8 ms (ประมาณ 5.8 วินาทีต่อภาพ)** ในขั้นตอน M7 ของตารางที่ 3.8 และสอดคล้องตรงกันกับค่าเฉลี่ยเฉพาะภาพที่ 2 ถึง 7 ของตารางที่ 3.7 (34,942 ms / 6 ภาพ = **5,823.7 ms หรือ 5.82 วินาทีต่อภาพ**)
...
4. **การจำแนกสัดส่วนเวลาการประมวลผลภายในระบบ (Internal Profiling Breakdown):**
   4.1 *SigLIP2 Zero-shot Verification:* ใช้เวลาเฉลี่ยประมาณ ~220–350 ms ในการสกัดเวกเตอร์คุณลักษณะและคำนวณความน่าจะเป็นของ 4 คลาส
   4.2 *YOLO26m Multi-hypothesis Detection (12 รอบ):* ใช้เวลาคิดเป็นประมาณ 88–92% ของเวลาทั้งหมด (~400–450 ms ต่อหนึ่งรอบการประมวลผลภาพ 960x960 บน CPU)
...
&emsp;&emsp;&emsp;&emsp;4.1.2.1 **แบบจำลองพื้นฐาน (Baseline Single-pass 0°):** มีความแม่นยำเพียง **42.86%** (ช่วงความเชื่อมั่น 95% Wilson Score CI: $[15.82\%, 74.95\%]$)
&emsp;&emsp;&emsp;&emsp;4.1.2.2 **การเพิ่มการหมุนภาพ 4 ทิศทาง (Multi-angle Rotation):** ยกระดับความแม่นยำเป็น **57.14%** ($+14.28\%$)
&emsp;&emsp;&emsp;&emsp;4.1.2.3 **การเพิ่มการปรับคอนทราสต์แสง (Multi-contrast 12 สมมติฐาน):** ยกระดับขึ้นเป็น **71.43%** ($+14.29\%$)
&emsp;&emsp;&emsp;&emsp;4.1.2.4 **การเสริมการกรองเชิงเรขาคณิตและสี (Geometric & Color Bonus):** เสริม `is_vertical` และ `red_ratio` ทำให้ระบบตรวจจับและแยกตัวเลขได้ครบถ้วนจนบรรลุ **100.00%** ($[64.57\%, 100.00\%]$)
&emsp;&emsp;&emsp;&emsp;4.1.2.5 **กลไกความปลอดภัยและการตรวจทาน (Safety Guards & SigLIP2):** เสริม Flip Guard, Cross-check, และ SigLIP2 ช่วยสร้างกลไกตรวจสอบความถูกต้องโดยไม่สูญเสียความแม่นยำ
