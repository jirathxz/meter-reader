...
[บทที่ 1: พื้นฐาน การออกแบบ และการประมวลผลภาพ ... 4](#บทที่-1-พื้นฐาน-การออกแบบ-และการประมวลผลภาพ)  
	&emsp;[1.1 ทฤษฎีและหลักการพื้นฐาน ... 4](#11-ทฤษฎีและหลักการพื้นฐาน)  
		&emsp;&emsp;[1.1.1 คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ ... 4](#111-คอมพิวเตอร์วิทัศน์และการตรวจจับวัตถุ)  
		&emsp;&emsp;[1.1.2 การจำแนกประเภทภาพแบบซีโร่ช็อต (Zero-shot Classification) ... 4](#112-การจำแนกประเภทภาพแบบซีโร่ช็อต-zero-shot-classification)  
		&emsp;&emsp;[1.1.3 การประมวลผลภาพดิจิทัล ... 4](#113-การประมวลผลภาพดิจิทัล)  
	&emsp;[1.2 การฝึกแบบจำลองสำหรับการตรวจจับตัวเลข ... 5](#12-การฝึกแบบจำลองสำหรับการตรวจจับตัวเลข)  
		&emsp;&emsp;[1.2.1 การเตรียมชุดข้อมูลจาก Roboflow ... 5](#121-การเตรียมชุดข้อมูลจาก-roboflow)  
			&emsp;&emsp;&emsp;[ขั้นตอนการลงทะเบียน Roboflow และการรับ API Key ... 5](#ขั้นตอนการลงทะเบียน-roboflow-และการรับ-api-key)  
		&emsp;&emsp;[1.2.2 การฝึกแบบจำลอง YOLO ... 5](#122-การฝึกแบบจำลอง-yolo)  
	&emsp;[1.3 สภาพแวดล้อมและการจัดการโครงงานด้วย `uv` ... 6](#13-สภาพแวดล้อมและการจัดการโครงงานด้วย-uv)  
		&emsp;&emsp;[1.3.1 โครงสร้างไฟล์ของระบบ ... 6](#131-โครงสร้างไฟล์ของระบบ)  
		&emsp;&emsp;[1.3.2 การสร้างสภาพแวดล้อมและการติดตั้งไลบรารี ... 6](#132-การสร้างสภาพแวดล้อมและการติดตั้งไลบรารี)  
	&emsp;[1.4 การคัดกรองภาพด้วยแบบจำลอง SigLIP2 ... 7](#14-การคัดกรองภาพด้วยแบบจำลอง-siglip2)  
	&emsp;[1.5 การตรวจจับและอ่านค่าตัวเลขด้วยคอมพิวเตอร์วิทัศน์ ... 7](#15-การตรวจจับและอ่านค่าตัวเลขด้วยคอมพิวเตอร์วิทัศน์)  
		&emsp;&emsp;[1. การหมุนภาพและการปรับปรุงคุณภาพภาพ ... 7](#1-การหมุนภาพและการปรับปรุงคุณภาพภาพ)  
		&emsp;&emsp;[2. การแปลงพิกัดจุดเดี่ยว (`remap_point`) และกล่องขอบเขต (`remap_bbox`) ... 7](#2-การแปลงพิกัดจุดเดี่ยว-remap_point-และกล่องขอบเขต-remap_bbox)  
		&emsp;&emsp;[3. การกรองแถวตัวเลขแนวตั้งด้วยฟังก์ชัน `is_vertical` ... 8](#3-การกรองแถวตัวเลขแนวตั้งด้วยฟังก์ชัน-is_vertical)  
		&emsp;&emsp;[4. การค้นหาทิศทางและฟิลเตอร์ที่เหมาะสมด้วยฟังก์ชัน `detect_digits_best` ... 8](#4-การค้นหาทิศทางและฟิลเตอร์ที่เหมาะสมด้วยฟังก์ชัน-detect_digits_best)  
[บทที่ 2: การพัฒนาแอปพลิเคชันและส่วนติดต่อผู้ใช้ ... 10](#บทที่-2-การพัฒนาแอปพลิเคชันและส่วนติดต่อผู้ใช้)  
	&emsp;[2.1 การบูรณาการระบบประมวลผลหลักและกลไกความปลอดภัย ... 10](#21-การบูรณาการระบบประมวลผลหลักและกลไกความปลอดภัย)  
		&emsp;&emsp;[1. การตรวจจับภาพกลับหัวแบบสมมาตรด้วยฟังก์ชัน `flip_guard` ... 10](#1-การตรวจจับภาพกลับหัวแบบสมมาตรด้วยฟังก์ชัน-flip_guard)  
		&emsp;&emsp;[2. ฟังก์ชันประมวลผลหลัก `read_meter` ... 10](#2-ฟังก์ชันประมวลผลหลัก-read_meter)  
	&emsp;[2.2 การพัฒนาส่วนเชื่อมต่อโปรแกรมประยุกต์ด้วย FastAPI ... 11](#22-การพัฒนาส่วนเชื่อมต่อโปรแกรมประยุกต์ด้วย-fastapi)  
	&emsp;[2.3 การพัฒนาส่วนติดต่อผู้ใช้ด้วย Gradio ... 11](#23-การพัฒนาส่วนติดต่อผู้ใช้ด้วย-gradio)  
	&emsp;[2.4 การทดสอบระบบและแนวทางการแก้ไขปัญหา ... 12](#24-การทดสอบระบบและแนวทางการแก้ไขปัญหา)  
		&emsp;&emsp;[วิธีการสั่งทำงานระบบด้วย `uv` ... 12](#วิธีการสั่งทำงานระบบด้วย-uv)  
		&emsp;&emsp;[แนวทางการวิเคราะห์และแก้ไขปัญหา (Debugging Matrix) ... 12](#แนวทางการวิเคราะห์และแก้ไขปัญหา-debugging-matrix)  
	&emsp;[2.5 ข้อควรพิจารณาสำหรับการนำระบบไปใช้งานจริง ... 13](#25-ข้อควรพิจารณาสำหรับการนำระบบไปใช้งานจริง)  
[อภิธานศัพท์ (Glossary) ... 14](#อภิธานศัพท์-glossary)  
	&emsp;[1. หมวดปัญญาประดิษฐ์และการเรียนรู้เชิงลึก ... 14](#1-หมวดปัญญาประดิษฐ์และการเรียนรู้เชิงลึก-ai--deep-learning)  
	&emsp;[2. หมวดการประมวลผลภาพดิจิทัล ... 14](#2-หมวดการประมวลผลภาพดิจิทัล-digital-image-processing--opencv)  
	&emsp;[3. หมวดสถาปัตยกรรมซอฟต์แวร์และเว็บ ... 14](#3-หมวดสถาปัตยกรรมซอฟต์แวร์และเว็บ-software-architecture--web)  
...
[ภาคผนวก: มาตรการด้านความปลอดภัยและความเสถียรของระบบ ... 16](#ภาคผนวก-มาตรการด้านความปลอดภัยและความเสถียรของระบบ)  
	&emsp;[ก. มาตรการด้านความปลอดภัยของระบบ ... 16](#ก-มาตรการด้านความปลอดภัยของระบบ)  
	&emsp;[ข. มาตรการด้านความเสถียรและความน่าเชื่อถือ ... 16](#ข-มาตรการด้านความเสถียรและความน่าเชื่อถือ)  
	&emsp;[ค. โครงสร้างระบบไฟล์ของโครงงานปัจจุบัน ... 16](#ค-โครงสร้างระบบไฟล์ของโครงงานปัจจุบัน)  
...
&emsp;&emsp;&emsp;&emsp;ผู้ใช้งานอาจถ่ายภาพด้วยอุปกรณ์เคลื่อนที่ในมุมเอียง 90° หรือกลับหัว 180°

#### 1.1.0.2 แสงสะท้อนและตัวเลขเลือนราง

&emsp;&emsp;&emsp;&emsp;กระจกหน้าปัดมีแสงสะท้อน คราบน้ำ หรืออยู่ในสภาพแวดล้อมที่มีแสงสว่างน้อย

#### 1.1.0.3 ป้ายข้อมูลอื่นบนตัวเรือน

&emsp;&emsp;&emsp;&emsp;ตัวเลขวันที่ผลิตหรือหมายเลขซีเรียลที่พิมพ์ในแนวตั้งบนตัวถังมาตรวัดน้ำ

#### 1.1.0.4 ตัวเลขที่มีลักษณะสมมาตรเมื่อกลับหัว

&emsp;&emsp;&emsp;&emsp;กลุ่มตัวเลข $6 \leftrightarrow 9$, $2 \leftrightarrow 5$, และ $0, 1, 8$ ที่อาจทำให้อ่านค่าผิดพลาดเมื่อภาพกลับทิศทาง
...
##### 1.1.2.1 Zero-shot Classification

&emsp;&emsp;&emsp;&emsp;ความสามารถในการจำแนกประเภทภาพตาม “คำอธิบายข้อความภาษาธรรมชาติ” โดยไม่ต้องทำการฝึกฝนแบบจำลองใหม่ เพียงกำหนดข้อความเปรียบเทียบ เช่น `"water meter"`, `"electricity meter"`, `"not a meter"` แบบจำลองจะคำนวณความน่าจะเป็นของข้อความที่สอดคล้องกับภาพมากที่สุด จึงเหมาะสมอย่างยิ่งสำหรับใช้เป็นด่านคัดกรองภาพเบื้องต้นก่อนส่งเข้าสู่แบบจำลองหลัก

##### 1.1.2.2 Sigmoid ($\sigma$)

&emsp;&emsp;&emsp;&emsp;ฟังก์ชัน $\sigma(z)=1/(1+e^{-z})$ แปลง logit เป็นความน่าจะเป็นอิสระต่อคู่ (0–1) ใช้ใน SigLIP2 เพื่อให้แต่ละคู่ภาพ-ข้อความตัดสินใจแยกกัน ไม่ผูกกับผลรวมของคลาสอื่น

##### 1.1.2.3 SigLIP / SigLIP2

&emsp;&emsp;&emsp;&emsp;ตระกูลแบบจำลอง Vision-Language จาก Google ที่ฝึกด้วย Sigmoid Loss แทน Softmax (Zhai et al., 2023) โดย SigLIP2 เพิ่มความสามารถหลายภาษาและการเข้าใจเชิงตำแหน่ง/คุณลักษณะหนาแน่นให้แม่นยำยิ่งขึ้น (Tschannen et al., 2025)
...
&emsp;&emsp;&emsp;&emsp;ปรับแสงเฉพาะจุดบนช่องความสว่าง (L) ในระบบสี LAB ดึงรายละเอียดในเงามืดโดยไม่ทำให้ส่วนสว่างจ้าเกินไป — เหมาะกับภาพที่มีเงาตกเป็นหย่อมๆ

##### 1.1.3.2 Histogram Equalization (HistEq)

&emsp;&emsp;&emsp;&emsp;เกลี่ยความสว่างทั่วทั้งภาพบนช่อง Y ในระบบสี YCrCb — เหมาะกับภาพที่มืดทั้งภาพ
...
#### 1.5.1 การหมุนภาพและการปรับปรุงคุณภาพภาพ
##### 1.5.1.1 ปัญหา

&emsp;&emsp;&emsp;&emsp;ภาพถ่ายมีความเอียง ตัวเลขมีความคมชัดต่ำ หรืออยู่ในบริเวณที่มีแสงสว่างไม่เพียงพอ ส่งผลให้ประสิทธิภาพในการตรวจจับตัวเลขของแบบจำลองลดลง

##### 1.5.1.2 แนวคิด

&emsp;&emsp;&emsp;&emsp;ดำเนินการทดสอบหมุนภาพ 4 ทิศทาง (0°, 90°, 180°, 270°) ร่วมกับการปรับปรุงคุณภาพแสง 3 รูปแบบ (ภาพต้นฉบับ, CLAHE, HistEq) รวมเป็น 12 รูปแบบการทดสอบ แล้วจึงให้แบบจำลองปัญญาประดิษฐ์ประเมินและคัดเลือกผลลัพธ์ที่มีความชัดเจนและน่าเชื่อถือสูงสุด

##### 1.5.1.3 ทำอย่างไร

&emsp;&emsp;&emsp;&emsp;ฟังก์ชัน `rotate_image` หมุนภาพด้วย OpenCV ส่วน `apply_prep` เลือกวิธีปรับแสง — โค้ดสั้นและทดสอบแยกได้
...
#### 1.5.2 การแปลงพิกัดจุดเดี่ยว (`remap_point`) และกล่องขอบเขต (`remap_bbox`)
...
##### 1.5.2.1 หมุน 90° (ตามเข็ม)

&emsp;&emsp;&emsp;&emsp;แกนสลับกัน จุด $(x, y)$ บนภาพหมุน จะตรงกับจุด $(y, H - x)$ บนภาพเดิม

##### 1.5.2.2 หมุน 180° (กลับหัว)

&emsp;&emsp;&emsp;&emsp;จุด $(x, y)$ จะตรงกับจุด $(W - x, H - y)$ บนภาพเดิม

##### 1.5.2.3 หมุน 270° (ทวนเข็ม)

&emsp;&emsp;&emsp;&emsp;แกนสลับกัน จุด $(x, y)$ จะตรงกับจุด $(W - y, x)$ บนภาพเดิม
...
#### 1.5.3 การกรองแถวตัวเลขแนวตั้งด้วยฟังก์ชัน `is_vertical`
##### 1.5.3.1 ปัญหา

&emsp;&emsp;&emsp;&emsp;ข้างตัวเรือนมิเตอร์มักมีตัวเลขวันที่ผลิต/ซีเรียลปั๊มเป็น **แนวตั้ง** ซึ่งไม่ใช่ค่ามิเตอร์ที่ต้องการอ่าน

##### 1.5.3.2 แนวคิด

&emsp;&emsp;&emsp;&emsp;หน้าปัดมิเตอร์จริงเรียงเป็น **แนวนอน** เสมอ — ระยะกว้างของแถวต้องมากกว่าระยะสูง ถ้าเจอแถวที่สูงกว่ากว้าง ให้ตัดทิ้ง

##### 1.5.3.3 ทำอย่างไร

&emsp;&emsp;&emsp;&emsp;วัดระยะซ้ายสุด–ขวาสุด (`width_span`) เทียบกับบนสุด–ล่างสุด (`height_span`) แบบ normalize ด้วยขนาดภาพ — ถ้า `height_span` มากกว่า `width_span` แบบมีนัยสำคัญ ถือว่าเป็นคอลัมน์แนวตั้ง
...
#### 1.5.4 การค้นหาทิศทางและฟิลเตอร์ที่เหมาะสมด้วยฟังก์ชัน `detect_digits_best`
...
##### 1.5.4.1 หมุน 4 ทิศ (0°/90°/180°/270°)

&emsp;&emsp;&emsp;&emsp;→ แก้ภาพถ่ายเอียงหรือกลับหัว

##### 1.5.4.2 ปรับแสง 3 แบบ (Orig/CLAHE/HistEq)

&emsp;&emsp;&emsp;&emsp;→ แก้เลขจางหรืออยู่ในเงามืด

##### 1.5.4.3 YOLO ตรวจจับ

&emsp;&emsp;&emsp;&emsp;→ หาตำแหน่งเลข 0–9 ทุกหลัก

##### 1.5.4.4 IoU dedup (threshold 0.45)

&emsp;&emsp;&emsp;&emsp;→ ตัดกล่องซ้อนที่ซ้ำกันให้เหลือกล่องเดียว

##### 1.5.4.5 is_vertical

&emsp;&emsp;&emsp;&emsp;→ ตัดป้ายวันที่/ซีเรียลที่เรียงเป็นแนวตั้ง

##### 1.5.4.6 red_ratio + Scoring

&emsp;&emsp;&emsp;&emsp;→ รู้ว่าทิศถูกหรือกลับหัว (แดงต้องอยู่ขวาสุด)

##### 1.5.4.7 Scoring (mean_conf × n)

&emsp;&emsp;&emsp;&emsp;→ เลือกรูปที่มั่นใจและครบหลัก

##### 1.5.4.8 Margin 0.12

&emsp;&emsp;&emsp;&emsp;→ กันสลับมุมเพราะคะแนนสูสี

##### 1.5.4.9 remap_bbox

&emsp;&emsp;&emsp;&emsp;→ วาดกล่องให้ตรงภาพต้นฉบับ


&emsp;&emsp;&emsp;&emsp;**การทำงานแบ่งออกเป็น 3 ฟังก์ชันย่อยตามหน้าที่ (แต่ละฟังก์ชันแก้ปัญหาไหน):**

##### 1.5.4.10 **`red_ratio(img_bgr, bbox)` — ตรวจสีแดงของหลักทศนิยม:**
###### 1.5.4.10.1 แก้ปัญหาอะไร

&emsp;&emsp;&emsp;&emsp;รู้ว่าภาพกลับหัวหรือไม่ — มิเตอร์น้ำจริงมีเลขทศนิยม **สีแดงอยู่ขวาสุด** เสมอ ถ้าแดงไปอยู่ซ้ายสุด แสดงว่าภาพน่าจะกลับหัว

###### 1.5.4.10.2 ทำอย่างไร

&emsp;&emsp;&emsp;&emsp;ครอปเฉพาะในกรอบ แปลงเป็น HSV แล้วนับสัดส่วนพิกเซลสีแดง


##### 1.5.4.11 **`eval_orientation(bgr_img, angle, prep)` — ประเมินคะแนนในแต่ละรูปแบบ:**
###### 1.5.4.11.1 วัตถุประสงค์

&emsp;&emsp;&emsp;&emsp;ตัดสินว่าทิศทางและฟิลเตอร์ใดมีความน่าเชื่อถือสูงสุด และกรองผลการตรวจจับที่คลาดเคลื่อน — โดยต้องเรียงตัวในแนวนอนและมีจำนวนหลัก 4–9 หลักจึงจะได้รับการคำนวณคะแนน

###### 1.5.4.11.2 วิธีการทำงาน

&emsp;&emsp;&emsp;&emsp;หมุนภาพและปรับคอนทราสต์ตามที่กำหนด → ตรวจจับตัวเลขด้วย YOLO → กำจัดกล่องที่ซ้ำซ้อน (`dedup_detections`) → ตรวจสอบการเรียงตัวแนวนอนและจำนวนหลัก → คำนวณคะแนน $\text{Score} = \text{Mean Confidence} \times \text{จำนวนหลัก}$ พร้อมปรับคะแนนตามตำแหน่งหลักทศนิยมสีแดง


##### 1.5.4.12 **`detect_digits_best(rgb_img)` — คัดเลือกผลลัพธ์ที่ดีที่สุด:**
###### 1.5.4.12.1 วัตถุประสงค์

&emsp;&emsp;&emsp;&emsp;คัดเลือกรูปแบบที่เหมาะสมที่สุดจาก 12 รูปแบบการทดสอบ โดยมีเกณฑ์ Margin ป้องกันความผันผวนของมุมองศา

###### 1.5.4.12.2 ทำอย่างไร

&emsp;&emsp;&emsp;&emsp;รัน `eval_orientation` ครบ 12 แบบ เลือกตัวแทนที่ดีที่สุดของแต่ละมุม (4 มุม) → หามุมที่คะแนนสูงสุด (`best_angle`) → ใช้ **Margin Rule** ถ้าชนะมุม 0° ไม่เกิน `ORIENT_MARGIN = 0.12` ให้ยึดมุม 0° ไว้ก่อน → แปลงพิกัดกล่องของมุมที่ชนะกลับสู่ภาพเดิมด้วย `remap_bbox`
...
#### 2.1.1 การตรวจจับภาพกลับหัวแบบสมมาตรด้วยฟังก์ชัน `flip_guard`
##### 2.1.1.1 ปัญหา

&emsp;&emsp;&emsp;&emsp;เลขอารบิกบางตัวสมมาตรเมื่อหมุน 180° เช่น $6 \leftrightarrow 9$, $2 \leftrightarrow 5$, $0,1,8$ ทำให้ภาพคว่ำก็ยังอ่านได้แต่เป็นคนละเลข

##### 2.1.1.2 แนวคิด

&emsp;&emsp;&emsp;&emsp;ลองหมุนภาพไปอีก 180° แล้วอ่านอีกรอบ ถ้าผลที่ได้ไม่สอดคล้องกับตารางสมมาตร `FLIP_MAP` แสดงว่าผลแรกอาจกลับหัว — ให้เตือนผู้ใช้แทนที่จะส่งเลขผิดไปเลย

##### 2.1.1.3 ทำอย่างไร

&emsp;&emsp;&emsp;&emsp;เทียบเลขแบบย้อนกลับ (`reversed`) กับผลจากการหมุน 180° ผ่าน `FLIP_MAP = {0:0, 1:1, 2:5, 5:2, 6:9, 8:8, 9:6}` ถ้าไม่ตรงและอีกฝั่งมั่นใจสูง จะตั้ง `warned = True`
...
#### 2.1.2 ฟังก์ชันประมวลผลหลัก `read_meter`
