...
3. **การแจกแจงกรณีข้อผิดพลาดจริง (Empirical Failure Taxonomy):** จากการวิเคราะห์เชิงลึกใน 39 ภาพที่อ่านไม่ตรงเฉลยครบทุกหลัก (บันทึกใน `reviews/test_set_evaluation.json`) พบสาเหตุหลัก 3 ประการ:
   * *ตัวเลขกึ่งกลางรอบหมุน (Half-turned Wheel Roll):* พบมากที่สุดถึง 64.1% (25 จาก 39 ภาพ) เช่น ภาพ `0b61cbb2...` เฉลยคือ `000108865` แต่ระบบอ่านได้ `000108863` เนื่องจากลูกล้อหลักสุดท้ายกำลังหมุนเปลี่ยนผ่าน ส่งผลให้ตัวเลขมีความกำกวมเชิงภาพ แม้ระบบอ่านถูกต้องถึง 8 ใน 9 หลักก็ตาม
   * *โครงสร้างตัวเลขคล้ายคลึงภายใต้คราบสกปรก (Structural Ambiguity):* พบ 25.6% (10 จาก 39 ภาพ) เช่น การสับสนระหว่างเลข 5 กับ 6 ในภาพ `0bf86f8b...` (อ่านได้ `00000165` จากเฉลย `00000155`) หรือการสับสนระหว่างเลข 8 กับ 9 ในสภาพแสงมืด
   * *แสงสะท้อนจ้าและสัญญาณรบกวนบนหน้าปัด (Specular Highlight & Noise):* พบ 10.3% (4 จาก 39 ภาพ) ส่งผลให้ตัวเลขเลือนรางหรือถูกบดบังบางส่วน
...
1. **โปรโตคอล ก (Initial Run with Cold-start Overhead — เฉลี่ย 10.3 วินาทีต่อภาพ):**
   * *ค่าที่วัดได้จริง:* เวลารวมทั้ง 7 ภาพเท่ากับ 72,339 มิลลิวินาที ส่งผลให้เวลาเฉลี่ยเท่ากับ **10,334 ms (10.33 วินาทีต่อภาพ)** ตามข้อมูลดิบในตารางที่ 3.7
   * *สาเหตุทางเทคนิค:* ภาพแรกสุดของการรัน (`meter_sample_01.jpg`) บันทึกเวลาสูงถึง **37,397 ms (37.4 วินาที)** เนื่องจากระบบมีภาระงาน Overhead ในการจัดสรรหน่วยความจำครั้งแรกของไดรเวอร์ซีพียูและเคอร์เนล PyTorch (First-inference allocation spike) ร่วมกับภาพดังกล่าวต้องหมุน 90 องศา เมื่อนำมารวมกับภาพที่เหลือ จึงดึงค่าเฉลี่ยของทั้งชุดขึ้นเป็น 10.3 วินาที
2. **โปรโตคอล ข (Repeated Benchmark Run — เฉลี่ย 8.3 วินาทีต่อภาพ):**
   * *ค่าที่วัดได้จริง:* ค่าเฉลี่ย **8.34 วินาทีต่อภาพ** (ช่วงเวลาต่ำสุด 4.9 วินาที, เฉลี่ย 8.3 วินาที, สูงสุด 26.7 วินาที) ตามที่รายงานในตารางที่ 3.2
   * *สาเหตุทางเทคนิค:* เป็นการรันชุดคำสั่ง `validate.py` ซ้ำในรอบถัดมาหลังจากโมดูลในหน่วยความจำและแคชพร้อมทำงานแล้ว ส่งผลให้เวลาของภาพแรกที่มีมุมเอียงลดลงจาก 37.4 วินาทีเหลือ 26.7 วินาที และดึงค่าเฉลี่ยของทั้งชุดภาพลดลงมาอยู่ที่ 8.34 วินาทีต่อภาพ
3. **โปรโตคอล ค (Steady-State Warm Run / Controlled Ablation — เฉลี่ย 5.8 วินาทีต่อภาพ):**
   * *ค่าที่วัดได้จริง:* ค่าเฉลี่ย **5,828.8 ms (ประมาณ 5.8 วินาทีต่อภาพ)** ในขั้นตอน M7 ของตารางที่ 3.8 และสอดคล้องตรงกันกับค่าเฉลี่ยเฉพาะภาพที่ 2 ถึง 7 ของตารางที่ 3.7 (34,942 ms / 6 ภาพ = **5,823.7 ms หรือ 5.82 วินาทีต่อภาพ**)
   * *สาเหตุทางเทคนิค:* เป็นเวลาที่สะท้อนภาระการคำนวณจริงในสภาวะการทำงานต่อเนื่องคงที่ (Steady-State) ของท่อประมวลผล 12 สมมติฐาน (4 มุม × 3 ฟิลเตอร์) โดยตัดสัญญาณรบกวนของ Cold-start Spike ออกไปอย่างสมบูรณ์

* **การจำแนกสัดส่วนเวลาการประมวลผลภายในระบบ (Internal Profiling Breakdown):**
  * *SigLIP2 Zero-shot Verification:* ใช้เวลาเฉลี่ยประมาณ ~220–350 ms ในการสกัดเวกเตอร์คุณลักษณะและคำนวณความน่าจะเป็นของ 4 คลาส
  * *YOLO26m Multi-hypothesis Detection (12 รอบ):* ใช้เวลาคิดเป็นประมาณ 88–92% ของเวลาทั้งหมด (~400–450 ms ต่อหนึ่งรอบการประมวลผลภาพ 960x960 บน CPU)
...
* **แนวทางการปรับปรุงความเร็วในระดับอุตสาหกรรม:** สำหรับการติดตั้งใช้งานจริงในระดับอุตสาหกรรม หากประมวลผลบนการ์ดจอแยกที่มีการประมวลผลแบบขนานด้วย CUDA และแปลงแบบจำลองเป็น TensorRT (FP16) เวลาประมวลผลของ 12 สมมติฐานจะสามารถลดลงเหลือต่ำกว่า ~150–250 ms ต่อภาพได้อย่างชัดเจน
