...
4. [บทที่ 3: การพัฒนาแอปพลิเคชันและส่วนติดต่อผู้ใช้](#บทที่-3-การพัฒนาแอปพลิเคชันและส่วนติดต่อผู้ใช้)
  ...
  4.4 [3.4 การทดสอบระบบและแนวทางการแก้ไขปัญหา](#34-การทดสอบระบบและแนวทางการแก้ไขปัญหา)
    ...
    4.4.4 [3.4.5 การประเมินแบบ End-to-End และโปรโตคอลทำซ้ำได้](#345-การประเมินแบบ-end-to-end-และโปรโตคอลทำซ้ำได้)
    4.4.5 [3.4.6 ผลการทดลองแบบ Ablation](#346-ผลการทดลองแบบ-ablation)
    4.4.6 [3.4.7 ผลการวัดเวลา](#347-ผลการวัดเวลา)
...
&emsp;&emsp;&emsp;&emsp;การนำเทคโนโลยีปัญญาประดิษฐ์ (Artificial Intelligence: AI) มาประยุกต์ใช้ในการอ่านค่ามาตรวัดน้ำจากภาพถ่ายจึงเป็นแนวทางที่ได้รับความสนใจอย่างแพร่หลายในระดับสากล โดยมีงานวิจัยที่รายงานผลเชิงบวกในชุดข้อมูลเฉพาะของตน เช่น Liang et al. (2022) และ Wang & Xiang (2024) ที่ทดสอบบนชุดข้อมูลควบคุมของงานนั้น ๆ — การสรุปว่า “ลดผิดพลาดอย่างมีนัยสำคัญ” โดยทั่วไปต้องอ้างผลทดสอบทางสถิติ (เช่น p-value หรือช่วงความเชื่อมั่น) บนชุดทดสอบอิสระ ซึ่งโปรเจกต์นี้ยังไม่มีการวัดแบบดังกล่าว จึงกล่าวได้เพียงว่ามีแนวโน้มที่น่าสนใจและต้องประเมินแบบควบคุมเพิ่มเติม อย่างไรก็ตาม ระบบส่วนใหญ่ยังมีข้อจำกัดในด้านการรับมือกับภาพที่มีคุณภาพต่ำ ภาพที่ถ่ายในมุมเอียง หรือภาพที่มีแสงสะท้อนรบกวน ซึ่งเป็นสภาพแวดล้อมที่พบได้บ่อยในการปฏิบัติงานจริง
...
1.3.1 เพื่อพัฒนาระบบอ่านค่ามาตรวัดน้ำอัตโนมัติจากภาพถ่ายที่สามารถทำงานได้บนทรัพยากรคอมพิวเตอร์ทั่วไป — ในที่นี้หมายถึงเครื่องที่ติดตั้ง Python 3.11 ขึ้นไป มี RAM 8GB ขึ้นไป ทำงานได้ทั้งโหมด CPU (ทดสอบบนเครื่องทั่วไป ได้เฉลี่ย 5–10 วินาทีต่อภาพ ที่ `YOLO_IMGSZ=960` 12 สมมติฐาน ตามหัวข้อ 3.4.7) และโหมด GPU NVIDIA CUDA (ต้องวัดบนเครื่องที่มีการ์ดจอแยกและระบุรุ่นก่อนอ้างตัวเลข)
1.3.2 เพื่อพัฒนาและประเมินกลไกสำหรับลดความผิดพลาดในการอ่านค่า โดยประยุกต์ใช้การคัดกรองภาพ (SigLIP2) และกลไกตรวจสอบความปลอดภัย (Safety Guards) — ประสิทธิผลเชิงปริมาณต้องวัดแบบควบคุมด้วยการเปรียบเทียบก่อน/หลังบนชุดทดสอบอิสระ (ดูหัวข้อ 3.4.3 และ 3.4.5–3.4.7)
...
**ขอบเขตด้านฮาร์ดแวร์:** ระบบได้รับการออกแบบให้ทำงานได้บนเครื่องคอมพิวเตอร์ทั่วไปที่ติดตั้ง Python 3.11 ขึ้นไป ทั้งโหมด CPU และโหมด GPU NVIDIA CUDA — ตัวเลขเวลาประมวลผลเชิงสังเกต (เช่น กรณีทดสอบ n=7 บน CPU ได้เฉลี่ย 10.3 วินาทีต่อภาพ ที่ `YOLO_IMGSZ=960` 12 สมมติฐาน) ได้ย้ายไปรายงานแบบควบคุมในหัวข้อ 3.4.7 พร้อมระบุฮาร์ดแวร์ รุ่น GPU ความละเอียดภาพ จำนวนสมมติฐาน สถานะ warm-up และวิธีวัดแล้ว จึงไม่ระบุตัวเลขในขอบเขตนี้เพื่อลดการเข้าใจผิด
...
| ระบบใช้ SigLIP2 `google/siglip2-base-patch16-224` แบบ zero-shot ด้วยการเปรียบเทียบภาพกับ 4 ข้อความพรอมต์ (`water meter`/`electricity meter`/`gas meter`/`not a meter`) | Implementation | `main.py:SIGLIP_MODEL` `METER_LABELS` `check_water_meter()` `METER_VERIFY_CONF=0.50` | Supported |
...
&emsp;&emsp;&emsp;&emsp;จากการประเมินเบื้องต้นบนชุดสาธิตจำนวน 7 ภาพ ระบบสามารถอ่านค่าได้ถูกต้องครบทุกภาพ อย่างไรก็ตาม ขนาดตัวอย่างยังไม่เพียงพอสำหรับสรุปประสิทธิภาพทั่วไปของระบบ จึงสรุปข้อจำกัดที่ควรคำนึงถึงดังต่อไปนี้:
