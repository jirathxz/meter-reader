...
| ตัวชี้วัด | ผลวัดเบื้องต้น | หมายเหตุเชิงวิชาการ |
|---|---|---|
| **Reading Success Rate** (อ่านสำเร็จไม่ว่าง) | **7/7 (100.0%)** | อัตราความสำเร็จเชิงระบบบนชุดสาธิต n=7 (ไม่ใช่ค่าความแม่นยำทั่วไป) |
| **Meter Verified Rate** (ผ่านคัดกรอง SigLIP2) | **7/7 (100.0%)** | `meter_check.verified=true` ทั้ง 7 ภาพผ่านเกณฑ์คัดกรองประเภทมาตรวัดน้ำ |
| **Avg Mean Confidence** (ความเชื่อมั่นเฉลี่ย) | **0.861** (min 0.816, max 0.891) | ค่าเฉลี่ยความเชื่อมั่นของแบบจำลอง (Mean Confidence) **ไม่ใช่ค่าความแม่นยำ (Confidence ≠ Accuracy)** |
| **Latency โปรโตคอล ข: Repeated Run** | **avg 8.3s / min 4.9s / max 26.7s** | เวลาเฉลี่ย 8.34s ต่อภาพจากการรันซ้ำทั้งชุดหลังหน่วยความจำพร้อม (AMD Ryzen 5, imgsz=960, 12 hyp) |
| **Latency โปรโตคอล ก: First Cold Run** | **avg 10.3s (72.3s รวมทั้ง 7 ภาพ)** | เวลาเฉลี่ย 10.33s ต่อภาพในรอบแรกสุด รวมผลกระทบ Overhead 37.4s ของภาพแรก (`meter_sample_01.jpg`) |
| **Latency โปรโตคอล ค: Steady-State Warm** | **avg 5.8s (5,824–5,829 ms)** | สภาวะคงที่ปราศจาก Cold-start Overhead (วัดเฉพาะภาพที่ 2 ถึง 7 ได้ 5.82s และตาราง 3.8 ขั้น M7 ได้ 5.83s) |

<p align="center"><em>ที่มา: ผลการทดสอบเบื้องต้นบนชุดภาพสาธิต (n=7) ในโครงงานนี้</em></p>

> [!WARNING]
> **ข้อจำกัดของขนาดตัวอย่าง (Sample Size Constraint, n=7):**
> ขนาดตัวอย่าง $n=7$ มีขนาดเล็กมากและเป็นเพียงชุดทดลองสาธิตเริ่มต้นเท่านั้น จึง**ห้ามสรุปว่าระบบมีค่าความแม่นยำทั่วไป (General Accuracy) เท่ากับ 100%** โดยเด็ดขาด เนื่องจากช่วงความเชื่อมั่นทางสถิติ Wilson Score 95% ของผล $7/7$ กว้างถึง **[64.6%, 100.0%]** ซึ่งสะท้อนว่าในประชากรภาพจริง ความแม่นยำอาจลดลงมาอยู่ที่ระดับ 65% ได้ การสรุปความแม่นยำระดับใช้งานจริงจำเป็นต้องทดสอบบนชุดข้อมูลมาตรฐานขนาดอย่างน้อย 50 ถึง 100 ภาพที่มีเฉลย Ground Truth ครอบคลุมสภาพแวดล้อมที่หลากหลาย
...
| อ่านค่ามิเตอร์ถูกครบทุกหลัก | วัด Exact Reading Accuracy / Digit-level Accuracy บน test set ที่มี ground-truth reading | **พิสูจน์ได้เฉพาะบนชุดสาธิตเบื้องต้น** — ได้ผล 7/7 ภาพ (100.0% [64.6%, 100.0%]) บนชุดสาธิต n=7 (หัวข้อ 3.4.5) **ห้ามสรุปเป็นความแม่นยำทั่วไป** และต้องขยายเป็น n≥50–100 ภาพพร้อมเฉลยเพื่อยืนยันทางสถิติ |
