...
```text
[ ภาพต้นฉบับ (กว้าง W, สูง H) ] ──(หมุนขวา 90°)──> [ ภาพที่หมุนแล้ว (กว้าง H, สูง W) ] ──(ตรวจจับพิกัด x, y)──> [ กล่องตัวเลขในภาพหมุน ] ──(remap_bbox)──> [ กล่องตัวเลขบนภาพต้นฉบับ ]
```
</details>
...
```text
[ ขั้นตอนที่ 0–1: นำเข้าและตรวจสอบภาพ ] ──> [ ขั้นตอนที่ 2: คัดกรองประเภทมาตรวัดด้วย SigLIP2 ] ──> [ ขั้นตอนที่ 3–4: ค้นหา 12 รูปแบบและแปลงพิกัด ] ──> [ ขั้นตอนที่ 5–6: ความปลอดภัยและการส่งออกผลลัพธ์ ]
```
</details>
...
```text
[ ผู้ใช้ถ่ายภาพมาตรวัดน้ำ ] ──> [ อัปโหลดผ่าน Gradio / REST API ] ──> { ตรวจสอบชนิดไฟล์และไบนารี PIL }
  ├─ (ไฟล์ไม่ถูกต้อง) ──> [ ปฏิเสธคำขอ HTTP 415/422 ]
  └─ (ไฟล์สมบูรณ์) ──> [ ส่งต่อไปยังขั้นตอนคัดกรอง (ขั้นตอนที่ 2) ]
```
</details>
...
```text
[ รับไฟล์ภาพที่ผ่านการตรวจสอบ ] ──> [ SigLIP2 Verification (Zero-shot 4 คลาส) ] ──> { เป็นภาพมาตรวัดน้ำ Conf ≥ 0.50? }
  ├─ (ไม่ใช่) ──> [ แจ้งเตือน: ไม่ใช่มาตรวัดน้ำ (ยุติการทำงานเพื่อประหยัด GPU) ]
  └─ (ใช่) ──> [ อนุมัติส่งต่อเข้าสู่การตรวจจับตัวเลข (ขั้นตอนที่ 3) ]
```
</details>
...
```text
[ ภาพมาตรวัดน้ำที่ผ่านเกณฑ์ ] ──> [ Multi-hypothesis Search 12 รูปแบบ ] ──> [ YOLO26 ตรวจจับ 0-9 + IoU Dedup ] ──> [ ตรวจกรอง is_vertical + กฎ Margin ] ──> [ แปลงพิกัดกล่องกลับ remap_bbox ]
```
</details>
...
```text
[ พิกัดกล่องตัวเลขบนภาพต้นฉบับ ] ──> [ Safety Guards Verification (flip_guard + SigLIP2 + align_ok) ] ──> [ สรุปผลลัพธ์ read_meter + วาด BBox ] ──> [ ส่งออกข้อมูล JSON และแสดงบน Web UI ]
```
</details>
...
```text
[ ภาพต้นฉบับ (W, H) ] ──(หมุนภาพตามองศา)──> [ ภาพที่หมุนแล้ว ] ──(ตรวจจับตัวเลข)──> [ กล่องตัวเลขในภาพหมุน ] ──(remap_bbox)──> [ กล่องตัวเลขบนภาพต้นฉบับเดิม ]
```
</details>
...
```text
[ จุด (x, y) ในภาพหมุน ] ──> { ตรวจสอบมุมองศา }
  ├─ (90°)  ──> [ คำนวณ (y, H - x) ] ──────────> [ พิกัดจุดบนภาพต้นฉบับ ]
  ├─ (180°) ──> [ คำนวณ (W - x, H - y) ] ──────> [ พิกัดจุดบนภาพต้นฉบับ ]
  └─ (270°) ──> [ คำนวณ (W - y, x) ] ──────────> [ พิกัดจุดบนภาพต้นฉบับ ]
```
</details>
...
```text
[ กล่อง [x1, y1, x2, y2] ในภาพหมุน ] ──> [ แปลงจุดมุมตรงข้ามด้วย remap_point ] ──> [ คำนวณ min/max ของแกน x และ y ] ──> [ กล่อง [X1, Y1, X2, Y2] บนภาพต้นฉบับ ]
```
</details>
...
```text
[ ระยะที่ 1: สร้างและประมวลผล 12 สมมติฐาน ] ──> [ ระยะที่ 2: คัดกรองเรขาคณิตและคำนวณคะแนน ] ──> [ ระยะที่ 3: ตัดสินผู้ชนะและแปลงพิกัดย้อนกลับ ]
```
</details>
...
```text
[ ภาพถ่ายต้นฉบับ ] ──> [ สร้าง 12 สมมติฐาน (4 ทิศ × 3 ฟิลเตอร์) ] ──> [ หมุนภาพ 4 ทิศ ] ──> [ ปรับคอนทราสต์ 3 แบบ ] ──> [ YOLO26 + IoU Dedup ]
```
</details>
...
```text
[ ข้อมูลกล่องตัวเลขแต่ละสมมติฐาน ] ──> { ตรวจสอบ is_vertical }
  ├─ (เป็นแนวตั้ง) ──> [ ตัดทิ้งทันที ]
  └─ (เป็นแนวนอน 4–9 หลัก) ──> [ คำนวณคะแนน Score ] ──> [ ตรวจสอบทศนิยมสีแดง red_ratio (+5% / -50%) ]
```
</details>
...
```text
[ คะแนนสรุปทั้ง 12 รูปแบบ ] ──> { เปรียบเทียบคะแนนกับมุม 0° (Margin Rule) }
  ├─ (ชนะเกิน 0.12) ────> [ เลือกมุมที่ได้คะแนนสูงสุด ] ──> [ แปลงพิกัดกล่องผู้ชนะ remap_bbox ]
  └─ (ชนะไม่เกิน 0.12) ─> [ รักษามุมเดิม 0° ไว้ ] ──────> [ แปลงพิกัดกล่องผู้ชนะ remap_bbox ]
```
</details>
...
```text
[ ผลลัพธ์ดิบจาก YOLO26 ] ──> [ กลไกความปลอดภัย 4 ระดับ (Flip / Cross-check / Align / Red) ] ──> [ ประเมินความเสี่ยงและแจ้งเตือน ] ──> [ ผลลัพธ์ที่ผ่านการตรวจสอบ ]
```
</details>
...
```text
[ ค่าอ่านได้ที่มุมชนะ ] + [ ค่าอ่านได้ที่มุม 180° ] ──> { เปรียบเทียบผ่านตาราง FLIP_MAP }
  ├─ (ตรงกัน / สอดคล้อง) ──────────────> [ ทิศทางภาพถูกต้องปกติ ]
  └─ (ขัดแย้ง & มุมตรงข้ามมั่นใจกว่า) ──> [ แจ้งเตือน: ภาพมีความเสี่ยงกลับหัว 180° ]
```
</details>
...
```text
[ ตัวเลขแต่ละหลักจาก YOLO26 ] ──> { ค่าความเชื่อมั่น Conf < 0.60? }
  ├─ (ไม่จริง ≥ 0.60) ──> [ ค่าตัวเลขน่าเชื่อถือสูง ]
  └─ (จริง < 0.60) ─────> [ ครอปเฉพาะภาพตัวเลขส่งให้ SigLIP2 จำแนกซ้ำ ] ──> [ ปรับปรุง Conf และบันทึกคำเตือน ]
```
</details>
...
```text
[ พิกัดกึ่งกลางแถวตัวเลข ] ──> [ คำนวณ Std Dev แนวตั้ง (≤ 0.10) ] ──> { แถวตัวเลขเรียงตรง? }
  ├─ (ไม่ตรง) ──> [ แจ้งเตือน: แถวตัวเลขเอียงผิดปกติ ]
  └─ (ตรง) ────> [ วิเคราะห์สัดส่วนสีแดง HSV ที่กล่องขวาสุด ] ──> { หลักแดงอยู่ขวา? }
                  ├─ (ใช่)    ──> [ ยืนยันตำแหน่งทศนิยมถูกต้อง (+5%) ]
                  └─ (ไม่ใช่) ──> [ แจ้งเตือน: ตรวจพบหลักแดงอยู่ซ้าย (-50%) ]
```
</details>
...
| 12 hypotheses ช่วยให้แม่นขึ้น | Ablation: `single-pass` vs `12-hypothesis` วัด Exact Reading Accuracy เดียวกัน | **พิสูจน์แล้ว (Supported)** — ผลการตัดทอน 8 ขั้นตอน M0 (42.9%) → M1 (57.1%) → M2 (71.4%) ในหัวข้อ 3.4.6 ยืนยันว่าการขยายสมมติฐานช่วยเพิ่มความแม่นยำอย่างต่อเนื่อง |
