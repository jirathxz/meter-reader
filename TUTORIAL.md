...
> 1. **Detector Test Set (194 ภาพ / 1,508 ตัวอย่าง):** คือชุดทดสอบทั้ง 194 ภาพที่กันออกจากการฝึก ใช้สำหรับประเมินสมรรถนะของตัวตรวจจับวัตถุ YOLO26m โดยตรง (mAP@50, mAP@50:95, Precision, Recall) ดังแสดงในตารางที่ 1.5 และตารางที่ 4.6
...
> 4. **คลาส Reading Digit ในชุดข้อมูล Roboflow:** ชุดข้อมูลต้นฉบับมีการกำหนดคลาสช่วยเสริม (Auxiliary Label) ชื่อ "Reading Digit" (มี 36 ตัวอย่างในชุดทดสอบ) สำหรับระบุตัวเลขที่กำลังหมุนเปลี่ยนผ่านกึ่งหลัก (Half-turned Wheel Roll) ทว่าในการอนุมานจริง ท่อประมวลผลหลักจะคัดกรองนำเฉพาะคลาสตัวเลข 0–9 มาเรียงลำดับเป็นค่าผลลัพธ์ของมาตรวัดน้ำ
...
<p align="center"><strong>ตารางที่ 1.5: ผลการประเมินประสิทธิภาพตัวตรวจจับวัตถุ YOLO26m บนชุดทดสอบอิสระ (Detector Test Set, n=194)</strong></p>
...
&emsp;&emsp;&emsp;&emsp;1) *ค่าที่วัดได้จริง:* ค่าเฉลี่ย 5,828.8 ms (ประมาณ 5.8 วินาทีต่อภาพ) ในขั้นตอน M7 ของตารางที่ 4.5 และสอดคล้องในเชิงขนาดรวมทั้งอยู่ในช่วงเดียวกันกับค่าเฉลี่ยเฉพาะภาพที่ 2 ถึง 7 ของตารางที่ 4.4 (34,942 ms / 6 ภาพ = 5,823.7 ms หรือ 5.82 วินาทีต่อภาพ)<br>
...
| **เฉพาะคลาสตัวเลข 0–9 (Classes 0–9 Only)** | **1,472** | **0.9336** | **0.9267** | **0.9297** | **0.9118** | **0.5086** |
| *Reading Digit (ป้ายกำกับช่วยเสริมใน Dataset)* | *36* | *0.9524* | *0.5556* | *0.7018* | *0.5469* | *0.1620* |
| **ภาพรวมทุกคลาสในชุดข้อมูล (All Classes)** | **1,508** | **0.9353** | **0.8930** | **0.9137** | **0.8786** | **0.4771** |
...
3. คลาสที่มีความท้าทายสูงและบทบาทของคลาสเสริม: คลาสตัวเลข `6` มีค่า Recall ต่ำกว่าคลาสอื่น (83.91%) เนื่องจากมีลักษณะโครงสร้างคล้ายคลึงกับเลข `8` และ `9` เมื่อเกิดการหมุนเอียงหรือมีคราบสกปรกบังเส้นขอบบางส่วน ขณะที่คลาส `Reading Digit` เป็นป้ายกำกับช่วยเสริม (Auxiliary Label) ที่มีอยู่ในชุดข้อมูล Roboflow สำหรับตัวเลขที่กำลังหมุนเปลี่ยนผ่านกึ่งหลัก (Half-turned Wheel Roll) ซึ่งมีตัวอย่างเพียง 36 ตัวอย่างและมีความกำกวมเชิงภาพสูง ส่งผลให้ได้ Recall 55.56% อย่างไรก็ดี ในท่อประมวลผลหลักของการอ่านค่ามาตรวัดน้ำ ระบบจะคัดกรองนำเฉพาะคลาสตัวเลข 0–9 มาใช้งานจริง ซึ่งเมื่อพิจารณาเฉพาะคลาสตัวเลข 0–9 (1,472 ตัวอย่าง) แบบจำลองมีค่า mAP@50 สูงถึง **91.18%** (Precision 93.36%, Recall 92.67%)
...
| YOLO26m มีประสิทธิภาพการตรวจจับในระดับที่น่าพอใจบนชุดทดสอบนี้ | Empirical | Precision 93.53%, Recall 89.30%, mAP@50 87.86% (และ 91.18% บนคลาส 0–9) บน Detector Test Set 194 ภาพ (ดูตารางที่ 4.6 ในหัวข้อ 4.1.5) | Supported |
